...
export const faqEntries: FaqEntry[] = [
  ...
  // ──────────────────── about the chatbot itself (meta) ────────────────────
  {
    ...
    answers: {
      ...
      'zh-TW': '我是 **Charles 親手打造的作品集小助手**，一個 corrective RAG 聊天機器人，專門用來展示他的 AI 工程能力。我本質上就是他設計的這套系統：一個 **LangGraph.js** 狀態機（分流 → 檢索 → 評估 → 生成），帶有會評估檢索結果、不足就改寫重試的**自我矯正迴圈**；**Qdrant** 混合檢索（dense + BM25, RRF 融合）接 **Voyage** cross-encoder 重排序；一個讓常見問題零生成成本就能回答的**語意快取**；以及一個成本分層的生成 stack（免費模型先答，較強的模型作為付費備援）。負責生成措辭的語言模型，只是這套系統裡可抽換的一個零件；真正定義我的，是 Charles 親手打造的檢索、矯正迴圈與成本分層。',
