...
export const faqEntries: FaqEntry[] = [
  ...
  // ──────────────────── about the chatbot itself (meta) ────────────────────
  {
    ...
    answers: {
      en: "I'm the **portfolio assistant Charles built from scratch**, a production corrective RAG system he made to put his AI engineering on display. Under the hood I'm a **LangGraph.js** state machine: a deterministic **triage** step and a **semantic FAQ cache** answer common questions with zero model calls, and everything else runs through **hybrid retrieval** over **Qdrant** (dense embeddings + BM25 sparse, fused with reciprocal rank fusion) followed by a **Voyage cross-encoder rerank**. A **self-correcting loop** then grades the retrieved context for relevance and automatically rewrites and retries the query when it falls short, before generating a grounded answer with inline citations. Generation runs on a **cost-aware cascade**: a free model first, with a stronger paid model as a **first-token-gated** backstop, so a fallback stays invisible to you. I'm hardened against prompt injection, indexed over his full blog, trilingual (English, Chinese, Japanese), and I run serverless on Vercel with a pipeline that re-indexes his content automatically. The language model writing this sentence is just one swappable part; what defines me is the retrieval, the corrective loop, and the cost tiers Charles engineered.",
      ...
      ja: '私は **Charles がゼロから自作したポートフォリオアシスタント**で、彼の AI エンジニアリングを実際に動くもので見せるために作られた production 級の corrective RAG システムです。中身は **LangGraph.js** のステートマシンです：deterministic な**トリアージ**と**セマンティック FAQ キャッシュ**がよくある質問をモデル呼び出しゼロで返し、それ以外は **Qdrant** 上の**ハイブリッド検索**（dense 埋め込み + BM25 sparse を RRF で融合）に続いて **Voyage の cross-encoder リランク**を通ります。さらに**自己修正ループ**が検索内容の関連性を評価し、不十分ならクエリを自動で書き換えて再試行し、最後に inline 引用付きの根拠ある回答を生成します。生成は**コスト階層型 cascade** で動きます：無料モデルが先に答え、強力な有料モデルは **first-token gated** のバックストップなので、フォールバックはあなたには見えません。私は prompt injection に対して堅牢で、彼のブログ全文を索引し、3 言語（English・中文・日本語）に対応し、Vercel 上で serverless として動作し、コンテンツを自動で再索引するパイプラインを備えています。この文章を書いている言語モデルは交換可能な一部品にすぎず、私を定義するのは Charles が設計した検索・自己修正ループ・コスト階層です。',
    },
  },
  {
    id: 'bot-who-are-you',
  ...
  },
  {
    ...
    answers: {
      en: "What sets him apart is the **AI Product Builder** model. A traditional PM hands a spec to engineering and waits; Charles sets the product strategy and then personally engineers the working prototype with AI (Claude Code, Codex), closing the loop from idea to shippable product about **5x faster**. That changes how he decides: he validates with a real, working prototype and live user signal before committing large resources, so each call rests on evidence. He still brings the full product toolkit (JTBD, RICE, opportunity framing, a North Star metric, outcomes over outputs), and he can build and ship the thing himself. The proof is concrete: at **USPACE** he doubled product-iteration velocity and leads three product lines, and on the side he has shipped **five AI products end to end**, including the multi-agent **Product Playbook** and this corrective-RAG chatbot. Strong opinions, loosely held, reprioritized fast when new signal contradicts the plan.",
      ...
      ja: '彼の独自性は **AI Product Builder** というモデルにあります。従来の PM は仕様をエンジニアに渡して待ちますが、Charles はまず製品戦略を立て、その上で AI（Claude Code、Codex）で動くプロトタイプを自ら作り、アイデアから出荷可能なプロダクトまでのループを約 **5 倍速**で回します。これは意思決定の仕方も変えます：大きなリソースを投じる前に、実際に動くプロトタイプと生のユーザー反応で検証するので、各判断は実証に基づきます。JTBD・RICE・機会フレーミング・North Star 指標・アウトプットよりアウトカムといった完全なプロダクト手法を備えつつ、自分でプロダクトを作って出荷もできます。証拠は具体的です：**USPACE** ではプロダクトのイテレーション速度を倍増させ 3 つのプロダクトラインを主導し、副業では **5 つの AI プロダクト**を端から端まで出荷しました（マルチエージェントの **Product Playbook** やこの corrective RAG チャットボットを含む）。strong opinions, loosely held、新しいシグナルが計画を覆せばすぐに優先順位を組み替えます。',
    },
  },
  {
    id: 'how-uses-ai',
