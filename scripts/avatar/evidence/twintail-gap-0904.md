...
**後續（同日）已排查，判定不需要修**：讀 `out/proportioned.vrm`（twintail
變形前）確認這批頂點對（如 Hair_Twintail_L prim0 v374/v448）變形前彼此只差
9–10mm，與後頸接縫同一種「近距離頂點、變形後被拉開」的形狀，但視覺驗證找
不到對應的可見瑕疵：`_tipprobe_tmp.html`（診斷用，未進版）在出貨檔 -9 上，
rest pose 兩側尾尖（受外套袖子部分遮擋處也單獨裁切放大）、`dance.vrma` 全
段 16 個時間點（0.5–8s，每 0.5s 一幀，含尾尖完整捲起的特寫裁切）、
`spin.vrma` 3 個時間點，肉眼逐張檢查全部乾淨——尾尖天生就是多股細髮束分岔
收束的地方，正常濃密陰影與描邊本身就很破碎，56–72° 的局部法向量偏差被淹沒
在既有的視覺複雜度裡，沒有讀出跟後頸接縫那種孤立、銳利、大片亮斑一樣的「不
