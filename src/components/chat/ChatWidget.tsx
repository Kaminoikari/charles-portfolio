        ...
        {status === 'streaming' && messages[messages.length - 1]?.text === '' && (
          <div className="flex items-center gap-2 self-start font-mono text-[11px] text-text-tertiary">
            <span className="flex gap-1">
              ...
              <span className="h-1.5 w-1.5 animate-chat-bounce rounded-full bg-accent-cyan [animation-delay:0.18s]" />
              <span className="h-1.5 w-1.5 animate-chat-bounce rounded-full bg-accent-cyan [animation-delay:0.36s]" />
            </span>
            {t('chat.thinking')}
          </div>
        )}
