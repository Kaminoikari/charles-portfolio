...
import { armRestSign, facingSign } from './avatarMode'
...
describe.each(FAMILIES)('procedural pitch on $id', (fam: Family) => {
  /**
   * Where the rig sits before anything is applied, in its own local space.
   *
   * `faceZ` is the way her face points, read off the eye against the head.
   * `herLeft` is the way her left is, read off her own two hands. Both come
   * from the body rather than from the version, so a body that disagrees with
   * its own metadata would show up here instead of being assumed away.
   */
  function rest(): { r: Rig; faceZ: number; herLeft: number; eye0: THREE.Vector3 } {
