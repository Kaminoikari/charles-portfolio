// The clearance of the Sendagaya Shino family: one body, declared in avatarVariants.ts
// as `sendagaya-shino` and not offered to visitors.
//
...
import { MEASURED } from './vroid-sendagaya-shino.measured.gen'
import { SIMULATED } from './vroid-sendagaya-shino.simulated.gen'
...
const DECISIONS: ClearanceDecisions = {
  ...
  // before offering `sendagaya-shino`: measure it the way the recipe says and replace
  // both fields.
  crownFringe: 0.0015,
  crownFringeMeasured:
  ...
  // This body's resting hair sits at 1.6451.
  pans: {
      peaceSign: { column: 0.02 },
      modelPose: { column: 0.05 },
      spin: { column: 0.09 },
      squat: { column: 0.06 },
      akimbo: { column: 0.06 },
      playFingers: { column: 0.06 },
      scratchHead: { column: 0.07 },
      idleLoop: { column: 0.06 },
      stretch: { waistUp: 0.1 },
      dance: { column: 0.18 },
    },
  ...
  waivers: {
      scratchHead: { handInHead: 0.72, handTop: 1.673 },
      dance: { handInHead: 0.19, reach: 0.744, hipsDrift: 0.15, endWrist: 1.27 },
