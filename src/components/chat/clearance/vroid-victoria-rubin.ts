// The clearance of the Victoria Rubin family: one body, declared in avatarVariants.ts
// as `victoria-rubin` and not offered to visitors.
//
...
import { MEASURED } from './vroid-victoria-rubin.measured.gen'
import { SIMULATED } from './vroid-victoria-rubin.simulated.gen'
...
const DECISIONS: ClearanceDecisions = {
  ...
  // before offering `victoria-rubin`: measure it the way the recipe says and replace
  // both fields.
  crownFringe: 0.0015,
  crownFringeMeasured:
  ...
  // This body's resting hair sits at 1.7373.
  pans: {
    peaceSign: { column: 0.12 },
    modelPose: { column: 0.14 },
    spin: { column: 0.18 },
    squat: { column: 0.15 },
    akimbo: { column: 0.15 },
    playFingers: { column: 0.16 },
    scratchHead: { column: 0.16 },
    idleLoop: { column: 0.15 },
    stretch: { waistUp: 0.18 },
    dance: { column: 0.27 },
  },
  ...
  waivers: {
    scratchHead: { handInHead: 0.61 },
    dance: { handInHead: 0.22, reach: 0.76, hipsDrift: 0.17, endWrist: 1.34 },
