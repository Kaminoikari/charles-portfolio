// The clearance of the VRoid hair sample, female family: one body, declared in avatarVariants.ts
// as `hair-female` and not offered to visitors.
//
...
import { MEASURED } from './vroid-hair-female.measured.gen'
import { SIMULATED } from './vroid-hair-female.simulated.gen'
...
const DECISIONS: ClearanceDecisions = {
  ...
  // before offering `hair-female`: measure it the way the recipe says and replace
  // both fields.
  crownFringe: 0.0015,
  crownFringeMeasured:
  ...
  // This body's resting hair sits at 1.6682.
  pans: {
    peaceSign: { column: 0.06 },
    modelPose: { column: 0.08 },
    spin: { column: 0.12 },
    squat: { column: 0.09 },
    akimbo: { column: 0.09 },
    playFingers: { column: 0.09 },
    scratchHead: { column: 0.09 },
