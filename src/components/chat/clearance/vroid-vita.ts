// The clearance of the Vita family: one body, declared in avatarVariants.ts
// as `vita` and not offered to visitors.
//
...
import { MEASURED } from './vroid-vita.measured.gen'
import { SIMULATED } from './vroid-vita.simulated.gen'
...
const DECISIONS: ClearanceDecisions = {
  ...
  // before offering `vita`: measure it the way the recipe says and replace
  // both fields.
  crownFringe: 0.0015,
  crownFringeMeasured:
  ...
  // This body's resting hair sits at 1.713.
  pans: {
      peaceSign: { column: 0.1 },
      modelPose: { column: 0.12 },
      spin: { column: 0.15 },
      squat: { column: 0.13 },
      akimbo: { column: 0.13 },
      playFingers: { column: 0.13 },
      scratchHead: { column: 0.13 },
