// The clearance of the VRoid sample C family: one body, declared in avatarVariants.ts
// as `sample-c` and not offered to visitors.
//
...
import { MEASURED } from './vroid-sample-c.measured.gen'
import { SIMULATED } from './vroid-sample-c.simulated.gen'
...
const DECISIONS: ClearanceDecisions = {
  ...
  // before offering `sample-c`: measure it the way the recipe says and replace
  // both fields.
  crownFringe: 0.0015,
  crownFringeMeasured:
  ...
  // This body's resting hair sits at 1.7561.
  pans: {
      peaceSign: { column: 0.14 },
      modelPose: { column: 0.16 },
      spin: { column: 0.27 },
      squat: { column: 0.18 },
      akimbo: { column: 0.18 },
      playFingers: { column: 0.18 },
      scratchHead: { column: 0.27 },
  ...
  waivers: {
      dance: { handInHead: 0.0, reach: 0.846, hipsDrift: 0.17, endWrist: 1.42 },
      spin: { reach: 0.81 },
      squat: { reach: 0.752 },
