// The clearance of the Rosa family: one body, declared in avatarVariants.ts as
// `rosa` and offered to visitors since 2026-09-26 in the place `twist` held.
//
// Mika's own VRoid project restyled in VRoid Studio 2.14.0 on 2026-09-26 at the
// owner's direction as Rosa (chest raised to 1.0) and exported as VRM 0.0. Two
// meta fields relaxed so the site may serve it (see the variant's comment),
// repacked with scripts/compress_vrm_webp.py, and scaled from crown 1.6199 to
// Mika's 1.582 with scripts/avatar/scalebody.py (x0.97660). The restyle moved
// bones, so rigOf hashes differently from every other family, and every number
// below was measured on the scaled file: evidence/rosa-0926.log.
//
...
import { MEASURED } from './vroid-rosa.measured.gen'
import { SIMULATED } from './vroid-rosa.simulated.gen'
...
const DECISIONS: ClearanceDecisions = {
  ...
  // a pass changed nothing: evidence/rosa-0926.log.
  pans: {
    spin: { column: 0.03 },
