...
// gets in. Both producers moved to the vtuber-kit repo on 2026-09-17, and the
// test that ran one as a subprocess went with them. What this holds is the
// consuming half: the shapes those files must have, and the combiner's refusal
// to pair measurements that were not taken on the same rig.
...
// `clearance producers` used to sit here: one test that ran
// `npx tsx scripts/measure-motions.ts --write` as a subprocess and checked the
// module it wrote carried the measured body's own rig sha rather than the
// family's. It moved to the vtuber-kit repo on 2026-09-17 with the producer it
// drives. What stays here is the consuming half: the shapes, and combine()'s
// refusal to pair producers run on different rigs.
