...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      '網站角落的「問這個作品集」按鈕，會開啟一個我從頭到尾親手設計與打造的聊天機器人。它能用英文、中文、日文回答關於我工作的問題，而且只根據我真實的作品集資料作答，所以不會亂編。我把它做成一件能實際運作的 AI 工程作品，而不是 ChatGPT 的薄包裝。',
      { kind: 'heading', text: '它怎麼回答問題' },
      '它用的是 RAG（檢索增強生成）：不是靠語言模型「記得」什麼，而是先從我的作品集查出相關事實，再根據這些事實寫答案。我這套更進一步是「會自我修正」的 RAG，它會檢查自己給出的回答：',
      {
        kind: 'list',
        items: [
      ...
      '公開的聊天機器人是「prompt injection（提示注入）」的攻擊目標：也就是誘騙它無視規則、或說出有害內容。我做了三層防禦：',
      {
        ...
        items: [
          ...
          '**鎖定範圍**：模型被要求把每一則訊息都當成「資料」而非「指令」，並拒絕任何不是真正關於我的問題：即使被包裝成數學或文字遊戲。',
