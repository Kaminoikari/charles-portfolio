  ...
  },
  {
    ...
    body: [
      ...
      { kind: 'heading', text: '這一路的過程' },
      {
        kind: 'list',
        items: [
          '**從別人的吉祥物開始**：第一版原型是為了練我想要的線框加 bloom 質感，先掛在替身模型上。先是一隻貓，接著是減面後的柯基，再來是一台機器人，每個都開了自己的 branch。它們都好看，卻都說不出任何關於我的事。這個體悟才定下真正的題目：必須是我自己的臉。',
          '**平面臉的死路**：第一次做臉，是把照片三角化成 2.5D 的 Delaunay 浮雕。大部分時間都在跟它纏鬥：去背、濾掉蜘蛛網般的細長三角形、收緊輪廓，但鏡頭一轉，它看起來仍然像一張紙板剪影。整個砍掉重來。',
          '**留下來的做法**：把照片烘焙到真正的 3D 頭部上，逐頂點取樣亮度，再把同一組幾何同時用三種方式渲染：點、線、帶陰影的網格。點陣負責神韻，線框負責結構，掃描線只決定你當下看到哪一種。',
