    ...
    body: [
      ...
      { kind: 'heading', text: `代價是什麼，誰可以豁免` },
      {
        ...
        items: [
          ...
          `她現在也有聲音了：23 句日文短句（VOICEVOX:春日部つむぎ 配音，頁尾有標注）涵蓋七種時刻，點她打招呼、打字送出時應答、進全螢幕、點建議問題、關閉時道別、回答完收尾、出錯時認錯，說話期間嘴型會跟著動（沿用回答時的口型迴圈，不做音訊分析）。語音第一下點擊就會出聲（全站背景音樂開關已同步移除），而且每句都只在訪客自己的點按裡播放，不會自己冒出聲音。英文版網站她會用同一把聲音講片假名英語，日式口音是刻意保留的萌點。`,
        ],
      },
