...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-08-21',
    title: `Mika 開口講中文了，英文也不再是用假名拼出來的`,
    tags: ['feature', 'technical'],
    body: [
      `她的中文語音以前並不存在。繁中站的訪客聽到的是未翻譯的日文原檔，英文站聽到的則是用片假名拼寫的英文，那是 VOICEVOX 唯一做得到的事，因為它沒有非日文的音素。現在三個語系各有自己的 25 句錄音，而且講的都是同一把聲音。`,
      { kind: 'heading', text: `先試了最直覺的做法，然後被耳朵否決` },
      `第一版是把她的聲音 clone 到一個多語模型，直接叫它講中文。中文帶著日本腔，英文更嚴重到聽不懂。問題不在參數：clone 的參考音全部是日文，模型對「這個人怎麼發中文的音」沒有任何證據，只能沿用日文的發音方式。用一個能分辨的測試確認過，同一個模型不套 clone 的時候中文講得很正常，所以拖累它的是參考音。`,
      { kind: 'heading', text: `口音向有口音的人買，音色向她買` },
      `定案的做法把一句話拆成兩件事分別取得。先用原生台灣腔的聲音把台詞唸出來，這一步拿到正確的咬字與語調，音色是別人的。接著做 voice conversion，逐幀改寫音色成為她的，而已經發出來的音不動。這是唯一能把口音跟音色拆開的操作。`,
      {
        kind: 'table',
        columns: [`階段`, `做什麼`, `買到什麼`],
        rows: [
      ...
      { kind: 'heading', text: `嘴型沒有跟著降級` },
      `她說話時的嘴型是預先烘焙、逐幀對上去的，資料原本來自 VOICEVOX 合成當下回報的逐母音時值。換掉合成器，照理會失去這份資料。結果新的合成器也給得出來，中文是逐字、英文是逐詞的起訖時間。而 voice conversion 不改變長度，逐檔實測轉換前後完全相同，所以這些時間在換完音色之後依然指向同一個字。`,
      `不過時間戳描述的是文字，文字跟聲音在頭尾會分家：四分之一的語音在最後一個字「結束」之後還響了最多 415 毫秒，還有兩句的開頭是沒有母音可對的鼻音（英文的 Mm、中文的嗯），嘴巴因此慢了將近半秒才動。所以每個檔的第一個與最後一個嘴型，改成釘在音訊本身量出來的起訖時刻上。50 個檔現在的開嘴與閉嘴，都落在她真正發出聲音的那一刻的一毫秒內。`,
      {
        kind: 'stats',
        items: [
      ...
      `笑聲沒有動：摸頭時的那三段えへへ不含任何語言，三個語系照舊共用同樣的檔案。例外是連續摸到第三次的時候，抱怨是話，所以三個語言各有一句。`,
    ],
  },
  {
    id: 'mika-motion-capture',
    date: '2026-08-19',
    title: `Mika 的手勢改成動作捕捉，每一個姿勢在你看到之前都被量過`,
    tags: ['feature', 'technical'],
    body: [
      `她的手臂手勢原本是人工敲進去的骨骼角度。拿真實骨架量過之後，十個裡有七個把手放在名字沒有描述的位置：一個叫做「摸頭髮」的動作，手其實停在髖部；一個戳臉頰的動作，指尖已經進到臉裡；比 YA 的時候兩隻手掌都背對著你。這十個全部拿掉了。她現在跳的是 VRoid Project 的動作捕捉，而新的測試會在一段動作接近你之前，先量出它到底把手放到哪裡。`,
      { kind: 'heading', text: `到底哪裡壞了` },
      `那些手勢唯一有過的自動檢查，是手臂橫向伸展有沒有超出畫布寬度。手最後落在哪、掌心朝哪、手指有沒有穿過臉，沒有任何測試看得見，所以每次調整都只能對著截圖用肉眼判斷，而在某一格看起來對的修正，會安靜地弄壞另一格。`,
      {
        kind: 'table',
      ...
      { kind: 'heading', text: `動作捕捉，而且只留通過檢查的` },
      `她現在跳的是 VRoid Project 官方動作檔裡的三支：在臉旁邊比 YA、一個安靜的站姿、原地轉一圈。這些動作的關節之所以協調，是因為真的有人這樣動過。同一包官方檔案裡還有四支被測試擋下來，這正是重點：官方資產一樣要通得過量測。`,
      {
        kind: 'list',
        items: [
          '`greeting` 起手時髖部比她的站立高度低 0.57m，前四秒她整個人從地板升起來，而且第六秒有一根指尖穿過臉頰。',
          '`shoot` 是差一步漏網的那一支。朝你舉起手指槍看起來乾乾淨淨，也已經通過認證等著發佈，直到採樣範圍從食指指尖擴大到整隻手的十六個關節。它的拇指有四分之一秒進到臉頰裡約 5mm。只量食指是 1.19，通過；量整隻手是 0.90，不通過。',
          '`showFullBody` 橫向伸到 0.713m，超出畫布本身，而且從頭到尾沒有一刻把掌心轉向你。',
          '`squat` 把髖部壓低 0.218m，幾乎貼到畫面底邊。',
        ],
      },
      { kind: 'heading', text: `量她的那層測試` },
      `新的驗證層直接從模型檔重建她的骨架，在 Node 裡算關節數學，不開瀏覽器也不需要 WebGL，所以把每支動作的每一格都量過大約一秒就跑完。有六件事必須成立：兩隻手的任何一個關節都不進到臉的體積裡、整個輪廓留在畫面上真的看得見的範圍內、一支以手為主角的動作必須在某個時刻把掌心轉向你、髖部離站立高度不超過 0.08m、首尾都站在自己腳上，以及骨架本身要解得出一個直立的身體。`,
      `最後那一條的存在，是因為它抓到了讀取器自己。三支動作裡有一支用不同的 rest 方向存骨骼，用直覺的方式讀會把她倒過來，腳在頭上。而在那個姿勢上，其他檢查全部照樣通過，因為一個對折的身體又窄又矮，掌心哪裡都不朝。每一道在被信任之前都先被弄紅過一次，而且要用不同的方式才弄得紅：把被拒絕的動作放回去，多數會各自因為對應的缺陷轉紅；直立那一道則只在拿掉解碼步驟時轉紅。上線的東西是一個測試結果。`,
      {
        kind: 'stats',
        items: [
      ...
      `她比較安靜的那些小動作沒有變：歪頭、四處張望、換重心、小跳一下、扭腰、低頭看地板，還有你摸她頭頂時的開心搖頭。那些只動頭和脊椎，本來就沒有手臂可以出錯。`,
    ],
  },
  {
    id: 'mika-full-height',
    date: '2026-08-14',
    title: `Mika 長到跟她正在進行的對話一樣高`,
    tags: ['design', 'technical'],
    body: [
      `Mika 原本是一個 342px 的人站在 560px 的面板旁邊，那個大小讀起來像擺在角落的裝飾，稱不上是正在跟你說話的嚮導。她現在在聊天室的兩種尺寸裡都佔滿高度：小視窗跟旁邊的面板等高，全螢幕則站滿整個螢幕。`,
      { kind: 'heading', text: `小視窗：與面板等高` },
      `她的畫布現在就是面板自己的高度，寬度依同比例跟著走，所以她手臂需要的空間在放大後仍然保留。構圖不變，仍是腰上景，**放大 1.64 倍**。面板與畫布在程式裡讀的是同一個數字，面板不可能單獨被改高。`,
      { kind: 'heading', text: `全螢幕：她自己的一欄` },
      `全螢幕原本讓她站在 236px 檢索管線側欄的底部，高度被鎖在 400px，結果她在全螢幕反而比剛放大過的小視窗還小。她改站到對話區右側，拿到 header 以下整個面板的高度。`,
      `把文字推開的只有她的身體。畫布其餘部分是留給手勢的透明空間，就浮在讓出來的空隙上，因此保住了 **132px** 的閱讀寬度，那是照整塊畫布開一個剛性欄位會吃掉的。`,
      `鏡頭為了這個更高的畫面重新構圖。舊構圖在她頭髮上方留了一段空白，在 400px 的畫布上看不出來，放到 800px 就是一個洞。上緣現在壓到她頭頂上方一點點，下緣維持在膝蓋，所以同一塊畫布把她**放大 10%**。`,
      { kind: 'heading', text: `視窗變小是把她縮小，不是把她搬走` },
      `沒有任何寬度會讓她跳回別的位置。她的尺寸取「面板高度」與「視窗寬度付得起的高度」較小者，比例固定，所以窄視窗得到的是站在同一個位置的小一號 Mika，每個手勢仍然完整在框內。矮視窗則是整個把她拿回來：她原本在高度低於 640px 時會消失，純粹因為那時她會壓在管線上，站到自己的欄位以後就沒有東西好壓了。`,
      { kind: 'heading', text: `建議問題不再消失` },
      `側欄原本一送出問題就把建議問題換成管線。那件事其實跟建議問題無關：她站在側欄腳下，兩者疊在一起會把最後幾站推到她頭後面。現在兩者共用側欄並且可以捲動，追問時不必等答案跑完才拿回那些提示。`,
    ],
  },
  {
