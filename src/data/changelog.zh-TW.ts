...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      `輸入框上方的造型列原本只有 Mika 的三種造型，現在有十三個：她的三種，加上另外十個 VRoid 角色。這十個角色在本月稍早就已經對每一支動作量測過，只是一直沒有放上造型列。點一下就會原地換上，和原本三種造型的換法一樣。十個都是女性角色；同一批量測的三個男性角色維持不上架。`,
      { kind: 'heading', text: `高個子角色的頭也留在畫面裡` },
      `全螢幕畫面的上緣只比 Mika 的頭頂高 25mm，而且只有播動作時鏡頭才會上移。新角色裡有八位比 Mika 高，其中三位靜止站著時頭頂就超出上緣。現在鏡頭靜止時，會在每個角色自己的頭頂上方留出和 Mika 一樣的空間；播動作時也不會低於這個高度。Mika 自己的構圖沒有變。`,
      `驗證方式：讓每個新角色在兩種畫面把每支動作都播一遍，每一幀都讀取畫布最上面兩排像素。靜止時沒有任何越界；播動作時剩下最多的，是某個角色跳舞到最高點時頭頂的一條細線。`,
      {
        kind: 'stats',
        items: [
          { value: '13', label: `可選造型` },
          { value: '10', label: `新角色` },
          { value: '0', label: `靜止時被切到的頭` },
        ],
      },
    ],
  },
  {
    id: 'mika-look-switch',
    date: '2026-09-03',
    title: `Mika 可以換造型了，而且網站會記住你選的那一個`,
    tags: ['feature', 'technical'],
    body: [
      `Mika 現在有三個造型：她一直穿著的粉髮重繪版、從 Blender 管線做出來的雙馬尾復刻版，以及她最初的紫髮 VRoid 原版。你看到哪一具，過去是寫死在程式碼裡的常數。現在輸入框上方多了一排小小的造型列，三個都在上面，點一下就原地換人。`,
      `復刻版是這份清單裡最有意思的一具，因為它是同一副 54 根骨架上的另一套網格：頭髮、髮飾、服裝都是在 Blender 裡建模出來的，另外兩具則是同一份匯出檔的貼圖重繪。骨架相同，正是三具都能原封不動沿用那十支動作的原因。`,
      { kind: 'heading', text: `換裝實際上做了什麼` },
      `VRM 把身體、臉、頭髮、衣服烘在同一個檔案裡，所以一套衣服就是另一個檔案。引擎在背景下載新檔，舊身體照常待機；解析完在同一幀裝上新身體，並重播 materialize 進場動畫，看起來像她再登場一次。十支動作從已解析的來源重新綁到新骨骼上，不會重新下載；表情與口型也直接延續，因為兩具身體帶著同一組 blendshape 名稱。`,
      `第一次載入與之後每一次切換，現在走的是同一條程式路徑。這是最主要的工程決定：如果切換另外寫一條，只要有人在其中一邊補了一個初始化步驟、忘了另一邊，兩邊就開始漂移，而沒有任何測試載得動一具身體來發現這件事。`,
      { kind: 'heading', text: `幾個小地方` },
      {
        kind: 'list',
        items: [
          `滑鼠移到或聚焦某個 chip 時，就先把那具身體暖進瀏覽器快取，真正點下去只剩解析。`,
          `身體在路上的時候，整排 chip 都會停用。下載到一半再點一次，只會多排一個已經沒意義的下載。`,
          `切換失敗時她留在原本的身體上，選取狀態跳回身體所在的那一格，而且什麼都不記。`,
          `你的選擇會在這個瀏覽器裡被記住，前提是它真的載入成功。帶 \`?mika=base\` 的連結會直接開在那具身體上，且不會動到這份記憶。`,
          `登記表裡的每具身體都被一條直接讀檔案的測試釘在同一副骨架與同一組表情名稱上，骨頭位移過的變體登記不進去。`,
        ],
      },
      {
        kind: 'stats',
        items: [
          { value: '3', label: `可選的身體` },
          { value: '1', label: `首次載入與每次切換共用的載入路徑` },
          { value: '34', label: `新增測試` },
        ],
      },
    ],
  },
  {
    ...
    date: '2026-08-27',
    title: `Mika 的中文改成辣妹語感，兩條守則跟著讓路`,
    tags: ['design'],
    body: [
      `上一版把她的個性寫進系統，這一版是本人下場改台詞。57 條常見問答的中文答案全部手寫重來，角色定位收斂成澀谷辣妹配上會照顧人的大姊姊。`,
      `有意思的地方在於，新的設定一寫下來，就跟上一版留下的機器檢查打架了。`,
      { kind: 'heading', text: `句尾的波浪號` },
      `新設定要求句子結尾適度用「～」，讓語氣有動態感。可是守則規定她的收尾句要落在語氣詞上，「～」接在語氣詞後面會被判成沒有語氣詞。`,
      `這條限制當初有它的道理。放寬之後，「哪一層想問都可以喔～，我告訴你它為什麼在那裡啦！」這種句子會同時騙過兩道檢查，讀起來像在模仿這個角色。後來另一道檢查被加寬到看得見那個「～」，漏洞就補起來了，只是沒有人回頭放寬第一道。這次放寬了，也驗過那句話依然會被擋下來。`,
      { kind: 'heading', text: `emoji 從「最多一個」變成一個都不留` },
      `舊規則是每則最多一個，還指名兩個場合要保留：擋掉攻擊、把私人問題交回給本人，理由是那裡的溫度就是內容。新設定把 emoji 整個列進紅線，所以那兩個場合也一起拿掉了。`,
      `判斷依據是訪客分不出回答自己的是哪一層。打「嗨」拿到的是寫死的招呼語，下一句問專案由模型回答，兩邊掛著不同的臉，同一個角色就有了兩張面孔。溫度改由文字本身承擔，而新的語氣本來就比舊的熱。`,
      { kind: 'heading', text: `笑聲不算語氣詞` },
      `有八條收尾句在語氣詞後面又掛了一個「嘻嘻！」，像「…喔～嘻嘻！」。「嘻嘻」過不了收尾規則，而新設定本來就把「嘻嘻～」列為開場招呼，所以那八句的笑聲搬到了句首。另外五句在句中疊了第二個語氣詞，改成各自獨立的句子。`,
      {
        kind: 'stats',
        items: [
          { value: '57', label: `條中文答案手寫重來` },
          { value: '16', label: `處為了通過守則微調` },
          { value: '9', label: `個 emoji 全部拿掉` },
        ],
      },
    ],
  },
  {
    id: 'mika-persona',
    date: '2026-08-26',
    title: `Mika 有了個性，她的聲音和文字終於是同一個人`,
    tags: ['feature', 'design'],
    body: [
      `Mika 一直有兩套語氣。你點她、送出問題、把面板放大，聽到的是一個很有精神的女生在講話：「講到 Charles，全世界就我最清楚啦！」可是她打字回答問題的時候，語氣是完全中性的，開場還自稱「Charles 的作品集小助手」。同一個角色，說話跟打字像兩個人。`,
      `這次我們把她的個性寫下來，並且讓每一條會產生回覆的路徑都用同一份定義。`,
      { kind: 'heading', text: `先盤點：她的語氣到底從哪裡來` },
      `我們把所有會產生回覆的地方列出來，總共十層。有些會經過 AI 模型，有些不會。`,
      `這個區分很重要，因為調整 AI 的指令只影響得到會經過模型的那幾層。招呼語、擋攻擊的回話，還有 57 條事先寫好的常見問答，都是直接把寫好的字丟給你看，模型完全不參與。`,
      `盤點之後，有三個地方是壞的：`,
      {
        kind: 'list',
        items: [
          `你問「我剛剛問了你什麼」，接手回答的是另一段程式，它從頭到尾沒被告知自己是 Mika，所以會退回成一個沒有名字的助理。`,
          `你打字說 hi，她自稱「作品集小助手」；你點她，語音第一句就自報名字。`,
          `57 條常見問答裡，只有 4 條是用她的視角寫的。`,
        ],
      },
      { kind: 'heading', text: `她是什麼樣的人` },
      `個性要跟外表對得起來，所以我們先看她長什麼樣子：紫黑漸層的長髮挑了一道青藍、紅藍交叉的髮夾、星星髮飾、黑色皮革項圈、深藍緞面棒球外套。這是日本地雷系混原宿辣妹的路線，當初挑配音也是照這個方向選的辣妹聲線。`,
      `所以個性的方向很清楚：**她是 Charles 的頭號粉絲兼經紀人，替他講他自己不好意思講的話。**`,
      `這個設定順便解決了作品集網站的一個老問題。會看這個網站的主要是招募方，一個人自己說「我做的產品觸及 700 萬人」聽起來像自誇，換成她來說就成立了。而且她答得準不準，本身就是這個網站正在展示的東西。`,
      { kind: 'heading', text: `為什麼她不能一路賣萌` },
      `如果只是叫 AI「講話可愛一點」，代價會直接落在可信度上。招募方要的是資訊密度。`,
      `所以個性是分層的。她的聲音只出現在回答的頭尾，中間那段維持原本的樣子：粗體的關鍵字、條列、每一個數字都在。`,
      {
        kind: 'table',
        columns: [`場合`, `個性強度`, `表現`],
        rows: [
          [`招呼、道別、擋攻擊`, `100%`, `完整的角色語氣`],
          [`問她自己是誰、怎麼做的`, `70%`, `開場自我介紹帶語氣，技術內容照舊嚴謹`],
          [`問經歷、專案、能力`, `20%`, `只在首句與末句帶語氣，中段維持簡報體`],
          [`婉拒、導向本人`, `40%`, `得體優先，語氣點到為止`],
        ],
      },
      `還有幾條反向規則：每則回答最多一個 emoji；作品集裡查不到的時候不放 emoji，直接說沒有；不准撒嬌式的自貶（「人家不知道啦」這種講法會直接毀掉招募方的信任）；引用規則跟拒答規則永遠大過語氣。`,
      { kind: 'heading', text: `日文踩到的兩個坑` },
      `三種語言的台詞是各自寫的。這沿用她配音時的做法，因為那種語感翻譯不過去。`,
      `日文有兩個問題，我們連續踩到三次才抓乾淨。`,
      `第一個是日文可以省略主語。「這個細節作品集裡沒有」這句話，日文寫出來完全不需要出現「我」，結果她的日文回答讀起來像旁白在介紹這個網站。更糟的是省略的主語會自動接到前一個說話的人身上：講他的專案時寫「為自己做的工具」，聽起來就變成她自己做的。現在只要省略主語會讓句子指錯人，我們就把它寫出來，她自稱也統一成「あたし」，跟會自報名字的那幾句配音一致。`,
      `第二個是語體。我們一開始判斷「說明事情就該收在禮貌形」，連她自己的開場句都寫成禮貌形，結果那句話讀起來像另一個有禮貌的陌生人。她的 25 句配音台詞全部是常體，所以現在的規則是：她自己的頭尾兩句一律常體，中間的說明維持禮貌形，語體的切換點落在「她講完、資料開始」的位置，那條線讀的人感覺得到。`,
      { kind: 'heading', text: `寫出來的句子，跟她講出來的句子` },
      `第一版的語氣句每一句都很短、都用第一人稱、也都通過測試，唸出來卻不對：那是書面語的句法，而她的錄音全是口說的。「這題我最愛回答，因為他的履歷密度真的高」是一篇短文；她配音裡講的是「講到 Charles，全世界就我最清楚啦！」判準最後定在她的配音本身，因為那 25 句是這個角色唯一真的被聽過的樣本，文字對不上錄音就等於換了一個人。`,
      `所以三種語言各有一組看得出來的標記：中文收在句尾助詞（喔、啦、欸）跟疊詞，日文用常體、開頭是語氣詞，英文全部縮寫、一個子句講完。164 條開場句與 171 條結尾句照這個標準重寫過。中間的內容只動過日文的三段，都是前面說的把「私」改成「あたし」。`,
      `另外有三句話從來不經過模型：她掛在角色旁邊的對話泡泡、面板打開時的那句邀請，還有請求失敗時補進她訊息裡的那句提示。這三句都是 review 找出來的。所以現在面板裡的每一條文案都得標明身分：她在講話、訪客要問的問題、或介面本身。漏標的會讓測試變紅。`,
      { kind: 'heading', text: `怎麼確保它不會再壞掉` },
      `每一件都有測試釘住，然後把修好的地方逐一重新弄壞，確認那些測試真的會變紅。連測試註解裡引用的數字本身也寫成斷言，過期的數字會讓測試變紅，不會留在那裡誤導下一個讀的人。`,
      {
        kind: 'stats',
        items: [
          { value: '10', label: `層語氣來源` },
          { value: '57', label: `條常見問答重新校過語氣` },
          { value: '3', label: `語各自改寫，不直譯` },
        ],
      },
    ],
  },
