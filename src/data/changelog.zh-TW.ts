...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-08-11',
    title: `助理可以放到全螢幕，順便把它找資料的過程攤開給你看`,
    tags: ['feature', 'design'],
    body: [
      `這個網站右下角的 AI 助理原本只有 400px 寬，一個回答加上它引用的來源就把版面塞滿了。這次補上全螢幕，並且用多出來的空間做一件小視窗放不下的事：把它找資料的每個步驟即時跑在左邊欄位上。`,
      { kind: 'heading', text: `三種大小，沒有關閉` },
      `現在有三個狀態：右下角的膠囊、原本的小視窗、蓋住整頁的全螢幕。三者彼此都能直接切換，按 Esc 會一次退一層，從膠囊點回來會回到你上次用的那個大小。`,
      `我刻意沒有做「關閉」。收起面板從來就不會清掉對話，那顆按鈕叫關閉等於在騙人，所以它改成「縮到最小」。真的要清空，按對話下面的清除。`,
      { kind: 'heading', text: `左邊那一欄在幹嘛` },
      `助理回答一個問題，背後跑了好幾個步驟：先判斷這句話該走哪條路，把問題轉成向量去搜，替搜到的內容打分數，覺得不夠好就改寫問題重搜，最後才生成回答。以前這些全部藏在一句「檢索中⋯⋯」後面。`,
      `全螢幕的左欄把每一步攤開。跑到哪一步就亮到哪，該步跑完就在旁邊印出自己花了幾毫秒，上方累加總時間。兩步之間有一道光沿著連線往下跑，代表資料真的從上一步交給了下一步。`,
      { kind: 'heading', text: `做到一半發現流程根本不是直線` },
      `我原本的設計稿是五個節點排成一直線，接上真實資料才知道畫錯了。有些問題在第一步就被判定成閒聊或常見問題，直接答完收工，完全不會碰到檢索。有些問題搜出來的東西不夠好，系統會自己改寫問題再搜一次，於是同一個步驟出現兩次。`,
      `所以左欄畫的是這次實際跑了什麼，不是固定的五格。重搜的那次會單獨列成一站，因為那正是這套檢索最值得看的行為：它會自己發現結果不夠好，然後重來一次。`,
      { kind: 'heading', text: `幾個小地方` },
      {
        kind: 'list',
        items: [
          `手機上的全螢幕是整片貼滿、沒有圓角，左欄收起來。理由很無聊：390px 的螢幕如果照桌機留 16px 邊距，全螢幕只比小視窗寬 8px，等於什麼都沒發生。`,
          `系統設定關閉動畫時，所有動畫一併停掉，節點名稱和耗時照樣完整顯示。資訊不能只活在動畫裡。`,
          `連線中斷或免費額度用完時，正在跑的那一步會標成「已中斷」。做的過程中它一度會永遠轉下去，因為我只處理了成功的路徑。`,
        ],
      },
    ],
  },
