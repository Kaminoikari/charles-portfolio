...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      `先前這個網站的 AI 助理,每收到一則訊息都當成獨立的一題在答:你問完「他在 USPACE 做什麼」再追問「那薪水呢」,它認不出「那」指的是同一個人,只能重新亂猜。這次補上兩塊功能:**對話記憶**讓它接得住追問,**多問拆題**讓一則訊息裡塞了好幾個問題時,能分開各自找資料、再合起來回答。`,
      ...
      `對話記憶用的是常見的 condense-question 做法:前端把最近幾輪對話帶上來,在進檢索前先花一次便宜的模型呼叫,把「那薪水呢」改寫成「Charles 在 USPACE 的薪水是多少」這種能獨立看懂的問句。重點是歷史只用在這一步,後面的檢索、FAQ 比對、生成拿到的都是一句自足的問題,最花錢的生成提示不會因為帶了歷史而變胖。沒有歷史的第一輪整個跳過,成本跟以前一樣。`,
