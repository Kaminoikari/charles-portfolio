    ...
    body: [
      ...
      { kind: 'heading', text: `代價是什麼，誰可以豁免` },
      {
        ...
        items: [
          ...
          `她現在也有聲音了：24 句日文短句（VOICEVOX:春日部つむぎ 配音）涵蓋八種時刻，首次開啟的自我介紹、點她打招呼、打字送出時應答、進全螢幕、點建議問題、關閉時道別、回答完收尾、出錯時認錯，說話期間嘴型會跟著動（沿用回答時的口型迴圈，不做音訊分析）。另外還有五段只有笑聲的短音檔，留給摸頭。語音第一下點擊就會出聲（全站背景音樂開關已同步移除），而且她發出的每個聲音都是在回應訪客剛剛做的動作，不會自己冒出聲音。英文版網站她會用同一把聲音講片假名英語，日式口音是刻意保留的萌點。笑聲沒有語言，所以三個語系聽到的是同樣那五段。每次造訪第一次打開聊天室她會完整自我介紹一次，之後就只講短句。`,
        ],
      },
