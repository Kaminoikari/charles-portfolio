...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-08-21',
    title: `Mika 開始講中文和英文，而且三種語言都是同一把聲音`,
    tags: ['feature', 'technical'],
    body: [
      `如果你之前把網站切到中文或英文，再讓 Mika 開口，你聽到的其實都是日文。中文站直接播沒有翻譯的日文原檔；英文站則是把英文單字先用日文的假名拼出來再念，聽起來不太像英文。這是當初配音軟體 VOICEVOX 的限制，它只發得出日文的音。這次我們替中文和英文各錄了 25 句，加上原本的日文，三個語言都有自己的台詞，而且聽起來是同一個人在講話。`,
      { kind: 'heading', text: `第一個方法：叫 AI 模仿她的聲音去講中文` },
      `一開始的想法很單純。有一種 AI 只要聽過一個人的錄音，就能模仿那個人的聲音，而且會講多國語言。我們把 Mika 的日文錄音餵給它，請它用她的聲音念中文。`,
      `結果不能用。中文帶著很重的日本腔，英文更糟，幾乎聽不出在講什麼。`,
      `我們原本以為是設定沒調好，後來才想通真正的原因：餵給 AI 的錄音全部都是日文。它從來沒聽過這個聲音怎麼發中文的音，只好照日文的方式硬念。為了確認，我們把「模仿她」這一步關掉，讓同一個 AI 用它預設的聲音念同一段中文，念得很標準。所以問題出在那些參考錄音上。`,
      { kind: 'heading', text: `後來的做法：先請人念對，再把聲音換成她的` },
      `最後我們把一句話拆成兩個步驟來完成。`,
      `第一步，找一個母語是台灣中文的語音把台詞念出來。這一步負責咬字和語氣，念完之後內容是對的，但聲音是別人的。`,
      `第二步，用 voice conversion 這項技術，把這段錄音的音色換成 Mika 的。它只改「聽起來像誰」，不改「念了什麼」，所以第一步辛苦調好的發音會原封不動保留下來。`,
      {
        kind: 'table',
        columns: [`階段`, `做什麼`, `買到什麼`],
        rows: [
          ['1', `用原生口音的聲線合成，同時取回逐字時間戳`, `正確的咬字與語調`],
          ['2', `voice conversion 換音色`, `她的聲音`],
          ['3', `時間戳換算成嘴型軌`, `逐幀對嘴`],
          ['4', `編碼成 24kHz AAC`, `跟既有語音同規格`],
        ],
      },
      { kind: 'heading', text: `換掉配音軟體之後，嘴型還對得上嗎？` },
      `Mika 講話時嘴巴的開合是事先算好的。我們會先做出一條時間表，再把它一格一格對到聲音上。這條時間表原本靠 VOICEVOX 提供：它在合成語音的時候，會順便告訴我們每個母音出現在第幾秒。`,
      `換掉 VOICEVOX，這份資料照理說就一起沒了。所幸新的合成工具也會給同樣的東西，中文報到每個字，英文報到每個單字。`,
      `還有一件事得先確認：第二步換音色，會不會讓聲音變長或變短。只要長度變了，整條時間表就會對不上。我們把每個檔案在換音色前後都量過，長度完全一樣，所以那些時間點換完之後指的還是同一個字。`,
      `不過這條時間表描述的是「文字」，而文字的起訖跟聲音的起訖並不一樣。`,
      `有四分之一的錄音，在最後一個字念完之後聲音還會繼續響，最長多出 415 毫秒。另外有兩句的開頭是鼻音，英文那句是 Mm，中文那句是嗯。鼻音沒有對應的嘴型可以做，所以她的嘴巴會等將近半秒才開始動。`,
      `修法是把每個檔案的第一個和最後一個嘴型，基準從文字的時間改成聲音本身真正開始和結束的時間。現在 50 個檔案裡，她張嘴和閉嘴的時機跟實際出聲的時機，相差都在千分之一秒以內。`,
      {
        kind: 'stats',
        items: [
          { value: '50', label: `新錄的語音` },
          { value: '0ms', label: `換音色造成的時間偏移` },
          { value: '3', label: `語系，同一把聲音` },
        ],
      },
      `笑聲的部分沒有改。摸她頭的時候會聽到三段「欸嘿嘿」，這種聲音本來就沒有語言的差別，三個語言共用同一批檔案。只有連續摸第三次時的那句抱怨是例外，因為抱怨是說出來的話，所以中英日各錄了一句。`,
    ],
  },
  {
    id: 'mika-motion-capture',
    date: '2026-08-19',
    title: `Mika 的手部動作換成動作捕捉，每個姿勢上線前都會被程式量過`,
    tags: ['feature', 'technical'],
    body: [
      `Mika 原本的十個手臂動作，是我們用手把每個關節的角度一個一個填進去做出來的，做完只能看截圖判斷像不像。這次我們寫了一支程式，把每個動作套到她真正的骨架上，實際算出她的手最後停在哪裡。`,
      `結果十個裡面有七個，手的位置跟動作名稱對不上。有個動作叫「摸頭髮」，她的手其實停在腰邊。有個動作要戳自己的臉頰，指尖已經穿進臉裡面。比 YA 的時候，兩隻手的手掌都朝外，觀眾只看得到手背。`,
      `這十個動作全部拿掉了。`,
      { kind: 'heading', text: `為什麼一直沒有人發現` },
      `那批動作從頭到尾只有一項自動檢查，就是手往兩側張開的時候會不會超出畫面寬度。`,
      `手最後停在哪裡、手掌朝哪個方向、手指有沒有穿過臉，這些程式都看不到，只能靠人盯著截圖判斷。`,
      `這個做法有個很難察覺的坑：一個動作是由很多格畫面組成的，你把正在看的那一格喬對了，同一個調整可能同時弄壞了另一格，而你根本沒看到。`,
      {
        kind: 'table',
        columns: [`手勢`, `它宣稱的`, `實際量到的`],
        rows: [
          ['`hairTouch`', `摸頭髮`, `手腕停在髖部，距頭部中心 0.567m`],
          ['`cheekPoke`', `戳自己的臉頰`, `指尖距頭部中心 0.090m，落在半徑 0.115m 的頭骨裡面`],
          ['`doublePeace`', `雙手比 YA`, `兩隻手掌都背對你，看到的是手背`],
          ['`salute`', `敬禮`, `手腕陷在頭髮裡，手指指向天花板`],
          ['`hipWave`', `一手插腰一手揮手`, `掌心與鏡頭平行，揮手只看得到手的側緣`],
        ],
      },
      { kind: 'heading', text: `改用動作捕捉，而且要先通過檢查才能上線` },
      `她現在的三個動作，來自 VRoid Project 官方提供的動作捕捉檔：在臉旁邊比 YA、安靜站著、原地轉一圈。動作捕捉的意思是先讓真人做出這些動作，把關節怎麼移動記錄下來，再套到角色身上，所以關節之間本來就是協調的。`,
      `同一包官方檔案裡，另外還有四個動作被我們的檢查擋下來。就算是官方做的，一樣要通過量測才會上線。`,
      {
        kind: 'list',
        items: [
          '`greeting`：一開始她的腰比站直時低了 0.57m，等於前四秒她是從地板裡慢慢升上來的。第六秒還有一根指尖穿過臉頰。',
          '`shoot`：這個差一點就上線了。她朝你舉起手指比槍的樣子看起來很乾淨，檢查也通過，已經排進上線名單。後來我們把量測範圍從食指指尖擴大到整隻手的十六個關節，才發現她的拇指有四分之一秒插進臉頰約 5mm。只量食指的分數是 1.19，算通過；量整隻手是 0.90，不通過。',
          '`showFullBody`：手往旁邊伸到 0.713m，已經超出畫面範圍，而且從頭到尾沒有一刻把手掌轉向觀眾。',
          '`squat`：蹲下去的時候腰壓低 0.218m，幾乎貼到畫面最下緣。',
        ],
      },
      { kind: 'heading', text: `負責檢查的那支程式在量什麼` },
      `這支檢查程式會直接讀她的模型檔，把骨架重建出來，再用數學算出每個關節的座標。它不需要打開瀏覽器，也不需要真的把畫面畫出來，所以把每個動作的每一格都算過，大約一秒就跑完。`,
      `它一共確認六件事：兩隻手的每個關節都不能進到臉的範圍裡；整個人要留在觀眾看得到的畫面內；如果這個動作的重點是手，那手掌一定要有某個瞬間轉向觀眾；腰的高度不能離站姿超過 0.08m；動作的開頭和結尾她要站在自己的腳上；還有骨架本身要能還原成一個站直的人。`,
      `最後一項會存在，是因為它抓到了我們自己讀取程式的錯誤。三個動作檔裡有一個，骨骼的預設方向跟另外兩個不一樣，如果照一般方式讀進來，她會變成頭下腳上倒立。`,
      `麻煩的是在那個姿勢下，其他五項檢查全部照樣通過。一個折起來的身體又窄又矮，手掌也沒有朝任何特定方向，剛好每一項都符合。`,
      `所以每一項檢查在我們相信它之前，都會先故意把它該抓的東西弄壞一次，確認它真的會失敗。弄壞的方法每一項還不太一樣：把被擋掉的動作放回去，多數檢查會各自抓到對應的缺陷；站直那一項則要把解碼那一步拿掉才會失敗。會出現在你眼前的動作，背後都有一份通過的量測結果。`,
      {
        kind: 'stats',
        items: [
          { value: '7 / 10', label: `舊手臂手勢實測是壞的` },
          { value: '3', label: `通過檢查上線的動捕` },
          { value: '6', label: `每支動作都要通過的檢查` },
        ],
      },
      `她比較安靜的小動作沒有變：歪頭、東張西望、換腳站、小跳一下、扭腰、低頭看地板，還有你摸她頭時開心的搖晃。這些動作只會動到頭和背，本來就沒有手臂會出錯。`,
    ],
  },
  {
    id: 'mika-full-height',
    date: '2026-08-14',
    title: `Mika 放大了，現在跟聊天視窗一樣高`,
    tags: ['design', 'technical'],
    body: [
      `以前你打開聊天視窗，會看到一個 342px 高的 Mika 站在 560px 高的面板旁邊。她比旁邊的面板矮一大截，看起來比較像放在角落的裝飾品，不太像正在跟你對話的人。`,
      `現在她在兩種聊天尺寸下都站滿高度：小視窗跟旁邊的面板一樣高，全螢幕則是整個螢幕的高度。`,
      { kind: 'heading', text: `小視窗：讓她跟面板一樣高` },
      `她的顯示區域比聊天面板更高，因為鏡頭在她頭髮上方留了一段空白，讓舉起來的手有地方伸。那段空白現在掛在面板的上緣之外，所以跟面板對齊的是她本人：髮頂貼著面板頂端，身體一路延伸到面板底部。`,
      `寬度照同樣比例放大，所以她揮手需要的左右空間，放大之後還在。`,
      `取景沒有變，一樣是從腰以上，只是畫得更大：大約是她當角落按鈕時的 **2.7 倍**。`,
      `程式裡她的大小是從面板高度算出來的。之後如果有人把面板調高，她會跟著長高，不會被漏掉。`,
      { kind: 'heading', text: `全螢幕：給她一個自己的欄位` },
      `全螢幕原本把她放在左邊那條 236px 寬的側欄下方，高度上限 400px。這造成一個很奇怪的結果：她在全螢幕反而比剛剛放大過的小視窗還小。`,
      `現在她移到對話區的右邊，可以用到標題列以下整個面板的高度。`,
      `這裡有個細節值得說明。會把文字推開的只有她的身體，她顯示區域的其他部分是透明的，留給手勢揮動用。這塊透明區域是浮在文字旁邊的空隙上，本身不佔位置。`,
      `如果偷懶一點，直接照整塊顯示區域切一個欄位出來，閱讀區會少掉 **132px** 的寬度。`,
      `鏡頭也重新取景過。舊的取景在她頭髮上方留了一段空白，在 400px 的小畫面上不太看得出來，但放大到 800px 就變成一塊明顯的空洞。`,
      `現在上緣壓到她頭頂上方一點點，下緣一樣停在膝蓋，所以在同一塊區域裡，她看起來比之前大 **10%**。`,
      { kind: 'heading', text: `視窗變小的時候，她會縮小，但不會換位置` },
      `不管你把視窗拉多窄，她都不會突然跳到別的地方去。`,
      `她的大小取兩個數字裡比較小的那一個：面板的高度，以及視窗寬度容得下的高度。她跟著面板走，所以矮視窗裡面板本身也矮，她就跟著一起矮。矮視窗不再多咬第二口：以前留給舉手的那段空白同樣必須待在畫面內，那會把她壓得比面板還矮，現在那段空白改成掛在面板上緣之外。要留在畫面內的是她的身體，手勢揮動的那片空白可以離開畫面。`,
      `視窗高度不夠的情況也一併處理了。以前只要視窗高度低於 640px，她就會整個消失，原因單純是那時候她會壓在側欄上面。現在她有自己的欄位，沒有東西好壓，也就不需要消失了。`,
      { kind: 'heading', text: `送出問題之後，推薦問題不會再被換掉` },
      `以前你一送出問題，側欄就會把推薦問題收起來，改成顯示檢索進度。`,
      `這件事其實跟推薦問題沒有關係。真正的原因是她站在側欄的腳邊，如果推薦問題和進度同時顯示，進度的最後幾格會被她的頭擋住。`,
      `她搬走之後就沒有這個限制了。現在兩者共用側欄，而且可以捲動，你想追問的時候，不必等答案跑完才拿回那些提示。`,
    ],
  },
  {
    id: 'hero-particle-field',
    date: '2026-08-14',
    title: `首屏卸下人臉，換上從中心往外擴散的粒子場`,
    tags: ['design', 'technical'],
    body: [
      `點雲人像已經從網站最上方退場。留下來的是那句標題，置中在一片由中心往外擴散的粒子裡。頁面現在直接進畫面，沒有載入序列，也沒有要先點過的門。`,
      { kind: 'heading', text: `為什麼開場動畫也一起走了` },
      `載入動畫、Enter 閘門與那段電影式開場，存在的理由全都是為了那張臉：閘門用來遮掩沉重的下載並在點擊中解鎖音效，開場是演在臉上的一段表演，靜態照片則是 WebGL 不可用時的替身。臉一旦拿掉，這三者就都沒事可做了，所以首屏改成掛載即渲染，標題在第一次繪製時就在。這同時讓網站少掉 **3.3MB** 的圖片、音訊與 3D 資產。`,
      { kind: 'heading', text: `重寫運動方式` },
      {
        kind: 'list',
        items: [
          `粒子原本生成在頭部網格的頂點上、往兩側剝離，那個形狀需要一顆頭才成立。現在每顆都從中心出發、筆直往外走，所以不論看畫面哪個位置，運動都是從一個點往外擴散。`,
          `粒子離中心的距離由它的年齡推導出來。舊的場是靠速度逐幀累加，於是第一幀只是中央一個點，要花完整一個生命週期才鋪得開；改成推導之後，第一次繪製就是穩定的滿場。`,
          `方向依面積抽樣。若讓每個角度分到一樣多的粒子，寬螢幕上下兩條短邊的密度會比四角高上約三倍。`,
          `每顆粒子能走多遠，是沿著它自己的方向量到畫框邊界決定的，所以朝向近邊的粒子不會再把大半生命耗在畫面外。`,
        ],
      },
    ],
  },
  {
    ...
    date: '2026-08-14',
    title: `Mika 進化：逐幀對嘴、即時表情、模型瘦身 64%`,
    tags: ['feature', 'design', 'technical'],
    body: [
      `登場隔天，**Mika** 學會了表演。她的嘴型現在逐音節貼合語音，臉上有真實的情緒起伏，會出聲打招呼、鞠躬道別，鏡頭也拉近到你真的看得見這些細節。整批升級同時讓她的下載量**輕了 64%**。`,
      { kind: 'heading', text: `口型同步，來自語音合成器自己的時間軸` },
      `每句語音現在都附帶一條預先烘焙的嘴型時間軸，來源是 VOICEVOX 合成該句時回報的逐母音精確時值。播放時引擎每一幀讀取音訊元素自身的時鐘，查出當下該張開哪個母音嘴型，五個嘴型通道彼此交叉淡變。成果是逐幀精準的對嘴，且完全零 runtime 音訊分析；瀏覽器從頭到尾沒有檢視音訊訊號，所以這套作法幾乎零成本，語音能播的地方它就能動。`,
      { kind: 'heading', text: `會演戲的臉和身體` },
      {
        kind: 'list',
        items: [
          `每個說話時刻都帶著表情：打招呼和道別時開心、接你的問題時放鬆、出錯時一臉抱歉。表情平滑進場，比語音多停留一拍，再緩緩退場。`,
          `手勢也加入了語彙：接問題時點頭、關掉聊天室時鞠躬。`,
          `語句之間她會呼吸、緩慢移動重心、眼神做小幅跳視，偶爾連眨兩下，所以她永遠不會凍結成假人。`,
          `三種狀態各有自己的視線節奏，中間由一道濾波銜接，所以她答完話回到待機時會花約 0.4 秒把頭轉回去。`,
        ],
      },
      { kind: 'heading', text: `電影感渲染與登場演出` },
      `渲染改用電影感（ACES）色調映射，並提高像素密度；回答時火星橘的輪廓光會亮起，腳下有柔和的接觸陰影把她定在地上，低位的青色補光把深色服裝從深色頁面中分離出來。她的登場也有了儀式感：青色粒子上升、短暫的光暈、伴隨一個小小的縮放彈跳，她就這樣成形。`,
      { kind: 'heading', text: `拉近到看得清楚` },
      `這些表演原本落在一張約 27px 高的臉上，手機更只剩 19px。鏡頭現在改取腰上景：臉放大 **1.7 倍**，她在頁面上佔的位置完全沒變，所以其他東西都不必讓位。打開聊天後她還會再大一些，畫布本身變大並以原生解析度渲染，面板態與全螢幕態皆是如此。下緣的裁切以漸層柔和地溶入頁面。畫面也放寬到容得下她的手臂：伸懶腰的指尖原本會被畫布邊緣切掉，現在三種擺放各自多了水平空間。視野角度是垂直計算的，所以這些空間完全不改變她看起來的大小。`,
      { kind: 'heading', text: `摸摸她的頭` },
      `在她頭頂來回輕撫幾下，或者在聊天室開著時點一下她的頭，她都會開心地小幅搖頭，還會害羞地笑一聲。連續三次，笑聲就會變成一句抱怨，用你正在看的那個語言講。放著她不管，大約每五秒她會從十六種小動作裡隨機挑一種表演。其中八種是會停住幾秒的姿勢：雙手比 YA、單手在臉旁比 YA、戳自己的臉頰、敬禮、指向你、雙手抱頭、單手插腰，以及插著腰揮手。另外八種比較安靜：歪頭、快速張望、端詳手掌、挪重心、小彈跳、撥頭髮、扭扭腰、低頭看地板。`,
      { kind: 'heading', text: `輕了 64%，什麼都沒少` },
      `模型貼圖重新打包為 WebP，所有 VRM 功能原封保留，骨架、表情、spring bone 資料經過逐位元組驗證。下載量從 15.4MB 降到 5.5MB（實際傳輸約 1.9MB），畫面上的角色則一個像素都沒變。`,
    ],
  },
  {
    id: 'avatar-guide-3d',
    date: '2026-08-13',
    title: `AI 助理有了全身 3D 角色`,
    tags: ['feature', 'design', 'technical'],
    body: [
      `右下角的 AI 助理現在有臉、有身體、也有名字：**Mika 醬**，一個全身 **3D 角色**，她本人就是啟動鈕，手機和桌機都看得到。她會即時反映對話狀態：沒事的時候左顧右盼；你一開始打字，她就上下點頭像在跟著你的草稿；答案串流進來時，嘴巴跟著母音口型開合，全身色調往本站的 mars orange 暖化，回答結束再緩緩恢復原色。在比手機寬的視窗開全螢幕時，她站在對話區旁邊，一邊「唸」答案、一邊看管線節點逐一亮起。`,
      { kind: 'heading', text: `一個 VRM 人形模型，由對話狀態驅動` },
      `角色是 **VRM** 模型（VTuber 工具鏈通用的人形格式），用 three.js 加 \`@pixiv/three-vrm\` 渲染。頭部動作是骨骼旋轉：頭、頸帶著一點脊椎一起轉，眼睛追著注視目標，所以她轉頭看東西的方式跟真人一樣。說話是五個日文母音口型的循環，節奏刻意不均勻，因為等拍開合看起來像機器。`,
      `這一切只靠聊天視窗本來就有的兩個訊號：輸入框裡有沒有字、答案是否正在串流。不追游標，也不分析音訊。`,
      { kind: 'heading', text: `代價是什麼，誰可以豁免` },
      {
        kind: 'list',
        items: [
          `模型 15.4MB，採 lazy 載入，而且等首頁開場動畫結束才開始抓，把頻寬留給開場。回訪由瀏覽器快取吸收。`,
          `開了 **prefers-reduced-motion** 或硬體沒有 WebGL2 的訪客，介面跟以前一模一樣：膠囊啟動鈕，原封不動。WebGL2 探測排在最後，這些訪客連一個 graphics context 都不用付。`,
          `角色本人就是啟動按鈕：aria-label 與鍵盤路徑原樣保留，focus ring 變成她腳下的光環。首次到訪會有一顆對話泡泡邀請你點她，8 秒後淡出，之後不再打擾。被豁免的訪客、載入失敗或拖太久時，由原本的膠囊按鈕代打；正常載入過程角落保持留白，直到她現身。`,
          `手機上她本人就站在角落當啟動鈕；聊天面板蓋住窄螢幕時她整個讓開，渲染同步暫停。`,
          `她現在也有聲音了：25 句日文短句（VOICEVOX:春日部つむぎ 配音）涵蓋九種時刻，首次開啟的自我介紹、點她打招呼、打字送出時應答、進全螢幕、點建議問題、關閉時道別、回答完收尾、出錯時認錯、連續摸頭第三次時抱怨一句，說話期間嘴型會跟著動（沿用回答時的口型迴圈，不做音訊分析）。另外還有三段只有笑聲的短音檔，留給前兩次摸頭。語音第一下點擊就會出聲（全站背景音樂開關已同步移除），而且她發出的每個聲音都是在回應訪客剛剛做的動作，不會自己冒出聲音。英文版網站她會用同一把聲音講片假名英語，日式口音是刻意保留的萌點。笑聲沒有語言，所以三個語系聽到的是同樣那三段。每次造訪第一次打開聊天室她會完整自我介紹一次，之後就只講短句。`,
        ],
      },
      `目前的角色是 VRoid Studio 官方樣本模型，自製角色完成前先代班。之後換角只需要換一個檔案。`,
    ],
  },
  {
    ...
    tags: ['design'],
    body: [
      `在這之前，點過 Enter 的訪客只要開場動畫一結束，背景音樂就會自動淡入。測試時覺得像個小彩蛋，實際上是替訪客做了決定：對方只想看網站，卻收到一段沒要求過的聲音。`,
      `現在音樂完全採 opt-in：左下角的喇叭按鈕是唯一的開關，圖示永遠如實反映播放狀態。點喇叭的那一下同時滿足 iOS Safari「聲音只能在真實手勢內啟動」的規則，所以在任何頁面、即使沒看過開場動畫，第一下就會有音樂。附帶的好處：網站不再替沒要求聲音的訪客掛著一條靜音串流。`,
    ],
  },
  {
    id: 'chatbot-fullscreen-pipeline',
    date: '2026-08-11',
    title: `助理新增全螢幕模式，把 RAG 檢索流程即時攤開`,
    tags: ['feature', 'design'],
    body: [
      `這個網站右下角的 AI 助理原本固定 400px 寬，一則回答加上引用來源就佔滿版面，使用者得在小框裡捲動長答案。這次補上全螢幕模式，多出來的空間用來做一件小視窗放不下的事：把助理背後的 **RAG 檢索流程**（retrieval-augmented generation，先查資料再生成答案）即時畫在左欄。這套流程是整個助理技術含量最高的部分，先前完整藏在一句「檢索中⋯⋯」後面。`,
      { kind: 'heading', text: `三種視窗狀態，刻意不做關閉鍵` },
      `現在有三個狀態：右下角的膠囊、原本的小視窗、蓋住整頁的全螢幕。三者互相直接切換，Esc 一次退一層，從膠囊點回來會回到你上次使用的尺寸。`,
      `「關閉」這顆按鈕我刻意拿掉。收起面板從來就不會清空對話，用「關閉」當標籤會讓使用者以為紀錄已經沒了，這是標籤與實際行為對不上。它現在叫「縮到最小」，要真的清空，對話下方有獨立的清除鍵。`,
      { kind: 'heading', text: `左欄顯示的是什麼` },
      `助理回答一個問題，背後會依序跑過幾個節點：**triage** 先判斷這則訊息該走哪條路，接著把問題轉成向量做檢索，替檢索結果評分，分數不夠就改寫查詢重跑，最後才生成答案。這種「結果不夠好就自己重來」的設計叫 **corrective RAG**。`,
      `左欄把每個節點列成一站。流程跑到哪裡就亮到哪裡，節點結束時在旁邊標出它花掉的毫秒數，頂端累加總耗時。兩站之間有一道光沿著連線往下走，代表資料實際交棒給了下一個節點。`,
      { kind: 'heading', text: `真實流程會分岔，也會自己重來` },
      `最初的設計稿把五個節點排成一直線，接上真實資料後才發現拓撲對不上。有些問題在 triage 就被判定成閒聊或命中 FAQ 快取，當場答完，完全不進檢索。有些問題檢索品質不佳，系統會改寫查詢再跑一次，同一個節點因此出現兩次。`,
      `所以左欄渲染的是這一次實際執行過的節點序列。重跑的那次檢索會單獨列成一站，因為那正是 corrective RAG 最值得展示的行為：它自己發現結果不夠好，然後重來一次。`,
      { kind: 'heading', text: `幾個實作取捨` },
      {
        kind: 'list',
        items: [
          `手機的全螢幕整片貼滿、不留圓角，左欄收起。390px 的螢幕若照桌機留 16px 邊距，全螢幕只會比小視窗寬 8px，使用者會覺得這顆按鈕沒有作用。`,
          `系統開啟 **prefers-reduced-motion** 時，所有動畫停用，節點名稱與耗時照常顯示。資訊不能只存在於動畫裡。`,
          `連線中斷或免費額度用盡時，執行中的節點會標記為「已中斷」。開發階段它一度會無限旋轉，因為當時只處理了成功路徑。`,
        ],
      },
    ],
  },
  {
    ...
    date: '2026-07-31',
    title: `讓助理記住剛才聊到哪，也拆得開一次問的好幾題`,
    tags: ['feature', 'technical'],
    body: [
      `先前這個網站的 AI 助理，每收到一則訊息都當成獨立的一題在答：你問完「他在 USPACE 做什麼」再追問「那團隊多大」，它認不出「那」接的是上一題的脈絡，只能重新亂猜。這次補上兩塊功能：**對話記憶**讓它接得住追問，**多問拆題**讓一則訊息裡塞了好幾個問題時，能分開各自找資料、再合起來回答。`,
      { kind: 'heading', text: `追問聽不懂、一次多問被稀釋` },
      `問題有兩種。一種是追問：代名詞、指稱（「他」「那個專案」「第二個」）一旦跨到上一輪，單題檢索根本不知道要找誰。另一種是一次問好幾題：整段被當成一個查詢丟進向量檢索，固定的回傳額度被幾個主題瓜分，冷門的那題常被擠掉，答案就缺一角。`,
      { kind: 'heading', text: `怎麼接住的` },
      `對話記憶用的是常見的 condense-question 做法：前端把最近幾輪對話帶上來，在進檢索前先花一次便宜的模型呼叫，把「那團隊多大」改寫成「Charles 在 USPACE 帶的團隊多大」這種能獨立看懂的問句。重點是歷史只用在這一步，後面的檢索、FAQ 比對、生成拿到的都是一句自足的問題，最花錢的生成提示不會因為帶了歷史而變胖。沒有歷史的第一輪整個跳過，成本跟以前一樣。`,
      `多問拆題則先過一道很省的判斷：一則訊息裡有兩個以上問號、或明顯的編號列舉，才會花一次免費層呼叫把它拆成幾個獨立子題。拆完後，檢索替每一題各發一次，再用輪流交錯的方式合併，讓每題都拿得到自己最相關的片段，同時把合併後的總量壓在上限內（目前最多四個子題、保留八段內容），生成提示一樣不會爆。單問（也就是絕大多數的流量）連那道判斷都是純字串比對，不多花任何一次模型呼叫。`,
      { kind: 'heading', text: `為什麼選這個折衷` },
      `這裡有個取捨。前沿實驗室對「一次多問」的標準做法，是讓模型自己把問題拆開、逐題檢索再彙整，品質最好，但每題都要一輪檢索，token 和呼叫數都往上加。這個助理跑的是省錢優先的路線：FAQ 快取加 Gemini 免費層先擋掉大半，加上流量本身很稀。所以我沒有無腦全拆，而是把前沿的拆題模式加上一道閘門，只在真的需要時才付那筆成本：常見的單問維持原價，追問和多問才多花一點點。`,
      { kind: 'heading', text: `對話記憶：為什麼不直接塞歷史` },
      {
        kind: 'table',
        columns: [`做法`, `修好檢索？`, `每輪 token`, `結果`],
        rows: [
          [`整段當獨立一題（先前）`, `否`, `最低`, `追問聽不懂`],
          [`把完整歷史塞進生成提示`, `否，只改到措辭`, `高，每輪都撐大提示`, `未採用`],
          [`Query contextualization`, `是`, `極低，只在追問時一次小呼叫`, `**採用**`],
        ],
      },
      { kind: 'heading', text: `多問處理：為什麼不每次都拆` },
      {
        kind: 'table',
        columns: [`做法`, `每題各自檢索？`, `單問成本`, `結果`],
        rows: [
          [`單包單次檢索（先前）`, `否，額度被瓜分`, `最低`, `多題品質差`],
          [`永遠先拆題`, `是`, `偏高，單問也多付`, `未採用`],
          [`閘門化拆題`, `是（偵測到多問才拆）`, `零額外`, `**採用**`],
        ],
      },
      `四道關卡都留了退路：閘門沒判到、拆題失敗、或某個子題檢索不到，一律退回原本的單題流程，不會因為多了新功能就把整個回答弄壞。分析數據也沒動，聊天記錄裡存的仍是使用者原本打的字，沒有換成改寫後的版本。`,
    ],
  },
  {
  ...
  },
  {
    ...
    tags: ['feature', 'technical'],
    body: [
      '網站上的 AI 聊天機器人現在能根據我部落格文章的全文回答問題，而不再只看標題與摘要。先前只有每篇文章的標題與副標進了索引，因此「這篇到底在論述什麼」這類需要讀進內文的問題往往無解。新的匯入流程會抓下每篇文章的內文，切成段落，再與其他內容一起放進檢索索引。',
      { kind: 'heading', text: '更新內容' },
      {
        kind: 'list',
        items: [
          '**全文檢索**：抓取程式會從 Substack 與 Medium 取得每篇文章的內文，存進納入版控的快取；這一步刻意與索引分離，讓重建索引時不必等待即時（且有速率限制）的抓取。',
          '**語言感知切塊**：一個零相依的遞迴切塊器把長文切成彼此重疊的段落，並同時辨識中文與英文的句子邊界。',
          '**原生多語**：文章以中文撰寫，但多語嵌入讓英文與日文的提問也能命中，更深入的答案因此在每一種語言都能浮現。',
          '**自家內容優先**：檢索時會調高作品集自家策展內容（關於、專案、經歷）的權重，讓它們排在外部文章之前，避免部落格的延伸討論蓋過網站第一手的說明。',
        ],
      },
    ],
  },
    ...
    body: [
      ...
      { kind: 'heading', text: '它如何運算' },
      {
        ...
        items: [
          ...
          '**輕觸即播的音效**：Enter 前導頁的存在，是為了讓開場音效能在你選擇進入的當下開始播放，因為瀏覽器只允許在使用者手勢之後才播放聲音。',
        ],
      },
    ],
  },
