...
export const changelog: ChangelogEntry[] = [
  {
    id: 'rag-chatbot',
    date: '2026-05-31',
    title: 'ポートフォリオ AI チャットボットを公開 — corrective RAG エージェント',
    tags: ['feature', 'technical'],
    body: [
      '右下の「AI とチャット」フローティングウィジェットは、私が一から設計・構築した三言語(英語 / 繁体字中国語 / 日本語)対応のチャットボットです。私の実際のポートフォリオデータのみに基づいて回答し、LLM の薄いラッパーではなく、本番品質の RAG を示す作品として意図的に設計しています。',
      { kind: 'heading', text: 'アーキテクチャ' },
      {
        kind: 'list',
        items: [
          '**LangGraph.js 上の corrective RAG** — ステートグラフ:トリアージ → 検索 → 評価 → 生成、自己修正のリライトループ付き。評価器が検索結果を不十分と判断すると、クエリを書き換えて再試行(上限あり)し、最後はハルシネーションせず正直に断ります。',
          '**Qdrant ハイブリッド検索** — dense(Voyage `voyage-3-large`、1024 次元、文書/クエリ非対称エンコード)+ Qdrant Cloud Inference がサーバーサイドで計算する BM25 sparse を RRF で融合し、Voyage `rerank-2.5` で再ランク。Supabase pgvector が無料枠の 2 プロジェクト上限に達したため移行しました。',
          '**二層生成** — Gemini 2.5 Flash(無料枠)が第 1 層、Claude が有料フォールバックで、Gemini が失敗しても訪問者は生成エラーを見ません。内部ステップ(評価・書き換え)は Gemini のみで有料予算を節約します。',
          '**エンティティグラフ** — 手動管理の隣接リストが 1〜2 ホップの関係(Charles ↔ 役割 ↔ プロジェクト ↔ ツール)を注入し、マルチホップの質問(「どのプロジェクトが Claude を使う?」)でチャンク分割だけでは見逃す関連を表面化します。',
        ],
      },
      { kind: 'heading', text: 'コスト管理 — ほとんどの質問は生成 LLM を呼ばない' },
      {
        kind: 'list',
        items: [
          '**決定論的トリアージ**(正規表現、約 0 ms、トークンゼロ)— プライバシーの質問(年齢・家族・給与など)は連絡先への丁寧な誘導、挨拶や問い合わせは定型回答。',
          '**セマンティックキャッシュ** — 約 46 の手書き回答トピック(×3 言語、数百の言い換え)を専用 Qdrant コレクションに埋め込み。訪問者の質問を一度だけ埋め込んで照合し、類似度が高ければ事前作成の回答をそのまま返す(生成 LLM 呼び出しなし)。本当に外れた時だけ完全な RAG に進みます。',
          '**三分類の評価** — 評価器が「回答可能 / 関連だが資料なし / 無関係」を分類し、無関係な質問(一般的な雑学)はループせず一度で断ります。',
        ],
      },
      { kind: 'heading', text: 'セキュリティ — プロンプトインジェクションへの多層防御' },
      {
        kind: 'list',
        items: [
          '**入力層** — LLM 呼び出し前にインジェクション/脱獄の形を検出・無害化:「指示を無視」、開発者モード、ロールプレイ/多重人格、そしてデコード/計算系(コード実行、base64/hex/バイナリ、出力に侮蔑語を隠す穴埋め・スペルアウトパズル)。',
          '**プロンプトのスコープロック** — 生成の system prompt がスコープを固定:すべての入力をデータとして扱い、数学/論理/言葉遊びに偽装されていても、私に関する本当の質問でなければ拒否します。',
          '**出力層** — 最終フィルタが、どう誘導されても侮蔑語(leet/空白の変種を含む)を含む回答を丸ごと破棄します。',
          '**運用面の堅牢化** — LLM 呼び出しの fail-fast(リトライ嵐なし)、呼び出しごとのタイムアウト、非ブロッキングの評価器で、サーバーレス関数がタイムアウトしないようにしました(以前は Gemini レート制限下で 504 を返していました)。',
        ],
      },
      { kind: 'heading', text: '運用' },
      'インデックス構築は手動トリガーの GitHub Action として実行します(ビルドには Voyage + Qdrant への外向き接続と 3 つの repo secret が必要)。コーパスはサイトを描画するのと同じ TypeScript データモジュールから抽出するので、ボットがポートフォリオの実際の記載から逸脱することはありません。Vercel 上にサイトと同一オリジンのストリーミング SSE エンドポイントとしてデプロイ。',
      'スタック:LangGraph.js · LangChain.js · Qdrant Cloud · Voyage AI · Gemini · Claude · React · Vercel。',
    ],
  },
  {
    ...
    body: [
      ...
      { kind: 'heading', text: '学んだこと' },
      {
        ...
        items: [
          ...
          '**設定値の集中管理が命綱** — `_config.py` + env override により、「CI では短い timeout」「ローカルテストでは 1 秒 timeout」が、コード変更ではなく 1 行の設定で済みます。',
