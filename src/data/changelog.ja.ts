...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      `彼女の中国語音声は、これまで存在していませんでした。繁体字サイトの訪問者が聞いていたのは翻訳されていない日本語クリップで、英語サイトではカタカナで綴った英語でした。VOICEVOX には日本語以外の音素がないため、それが唯一できることだったからです。今は三つのサイトがそれぞれ 24 句の録音を持ち、しかも同じ声です。`,
      ...
      `ただし時間情報が示しているのはテキストであり、テキストと音声は両端でずれます。全体の四分の一のクリップでは、合成器が「言い終えた」と申告した最後の文字のあとも最大 415 ミリ秒鳴り続けており、二句は冒頭が母音を持たない鼻音（英語の Mm、中国語の嗯）で、口が動き出すまで 0.5 秒近くかかっていました。そこで各クリップの最初と最後の口形は、音声そのものから計測した開始点と終了点に固定しています。48 本すべてが、実際に声が出ている瞬間から 1 ミリ秒以内で開閉するようになりました。`,
      {
        kind: 'stats',
        items: [
          { value: '48', label: `新規録音` },
      ...
      `笑い声はそのままです。頭をなでたときの三つのえへへには言語が含まれないので、三サイトとも同じファイルを再生します。`,
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      `頭の上を何度か往復してなでると、うれしそうに小さく頭を揺らして、照れたように笑います。言葉を話すのは、実際にタップしたときだけです。放っておくと、およそ 5 秒ごとに 16 種類の小さなしぐさからランダムにひとつ披露します。8 種は数秒とめて見せるポーズで、両手でピース、顔の横で片手ピース、自分のほっぺをつつく、敬礼、こちらを指さす、頭の後ろで手を組む、片手を腰に当てる、腰に手を当てたまま手を振る。残りの 8 種は、首かしげ、あたりを見回す、手のひらを眺める、重心移動、小さなバウンス、髪に触れる、腰ひねり、足元をのぞき込む、です。`,
    ...
    body: [
      ...
      { kind: 'heading', text: `コストと、免除される訪問者` },
      {
        ...
        items: [
          ...
          `声もつきました。24 の日本語ショートフレーズ（VOICEVOX:春日部つむぎ）が 8 つの瞬間をカバーします。初回オープンの自己紹介、タップへの挨拶、質問送信への相づち、フルスクリーン移行、おすすめ質問の選択、クローズ時のお別れ、回答完了の締め、エラー時の一言です。さらに、頭をなでたときの照れ笑いが 3 つあります。話しているあいだは口も動きます（回答時と同じ口形ループで、音声解析はしません）。音声は最初のタップからそのまま鳴ります（サイト全体の BGM スイッチは同時に廃止しました）。彼女が出す音はいつも訪問者の操作への返事で、勝手に鳴り出すことはありません。英語版サイトでは同じ声がカタカナ英語で話します。日本語なまりは意図的なチャームポイントです。笑い声は言葉を含まないので、どの言語版でも同じ 3 つが鳴ります。訪問して最初にチャットを開いたときだけフルの自己紹介をして、その後は短いフレーズに戻ります。`,
        ],
      },
