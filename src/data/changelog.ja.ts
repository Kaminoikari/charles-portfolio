...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      `これまでこのサイトの AI アシスタントは、メッセージを一通受け取るたびに独立した一問として処理していました。「USPACE で何をしたの?」と聞いた後に「じゃあ給料は?」と続けても、「それ」が誰を指すのか分からず、また一から推測するだけでした。今回は二つを追加しました。**会話メモリ**で追加質問を受け止められるようにし、**質問の分解**で、一通に複数の質問が入っていてもそれぞれ別々に検索してからまとめて回答できるようにしました。`,
      ...
      `会話メモリは一般的な condense-question の手法です。フロントエンドが直近の数ターンを送り、検索に入る前に安価なモデル呼び出しを一度だけ行って、「じゃあ給料は?」を「USPACE での Charles の給料は?」のように単独で意味の通る質問へ書き換えます。ポイントは履歴を使うのがこの一箇所だけということで、後続の検索、FAQ 照合、生成には自己完結した一つの質問が渡るので、いちばんコストのかかる生成プロンプトが履歴のせいで膨らむことはありません。履歴のない初回はこの工程ごとスキップし、コストは従来と同じです。`,
