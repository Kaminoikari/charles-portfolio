...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      'ヒーローポートレートの前に表示されるスプラッシュは、単なるパーセント表示に代わって、リズムのあるローディングシーケンスになりました。黒い画面の中央で発光するメビウスの帯が回転し、その下でステータス文がローテーションし、画面下部のヘアライン状のプログレスバーが 0 から満了まで走り切ってから、Enter コントロールが登場します。リズムは敬愛する二つのローダー、sidewave.it と aboutluca.com を参考に、本サイト自身のビジュアル言語で再構築しています。',
      { kind: 'heading', text: '変わったこと' },
      {
        kind: 'list',
        items: [
          '**リアルタイム描画のメビウスの帯**：ローディングシンボルは小さな canvas にパラメトリックなメビウスの帯をリアルタイムで描いたもので（sidewave は GIF で実装）、retina 画面でも鮮明なまま、背後のポートレートと同じワイヤーフレームと発光の言語を話します。',
  ...
  },
  {
    ...
    body: [
      ...
      { kind: 'heading', text: 'ここまでの道のり' },
      {
        kind: 'list',
        items: [
          '**始まりは他人のマスコット**：最初のプロトタイプは、目指していたワイヤーフレームとブルームの質感を練習するため、仮のモデルに掛けたものでした。猫、ポリゴンを間引いたコーギー、ロボットと、それぞれブランチを切って試しました。どれも見栄えは良いのに、私について何も語ってくれません。その気づきが本当のお題を決めました。自分の顔でやる、です。',
          '**平面の顔という袋小路**：最初の顔は、写真を 2.5D の Delaunay レリーフへ三角形分割する方式でした。背景の切り抜き、蜘蛛の巣のような細長い三角形の除去、輪郭の引き締めと、ほとんどの時間を格闘に費やしましたが、カメラが動いた瞬間、やはり紙の切り抜きにしか見えません。丸ごと破棄しました。',
          '**残った方式**：写真を本物の 3D の頭部に焼き込み、頂点ごとに明るさをサンプリングして、同じジオメトリを点、線、陰影付きメッシュの三通りで同時に描画します。点群が面影を、ワイヤーフレームが構造を担い、走査線はどちらを見せるかを決めるだけです。',
