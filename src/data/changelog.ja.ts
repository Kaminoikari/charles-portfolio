...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      ...
      `この演技はこれまで、高さ約 27px の顔の上で起きていました。スマートフォンでは約 19px です。カメラは腰から上を切り取る構図になり、顔は **1.7 倍**に。ページ上で彼女が占める面積はそのままなので、ほかの要素は動かす必要がありませんでした。チャットを開くとさらに大きくなり、キャンバス自体が広がってネイティブ解像度で描かれます。パネル表示でも全画面でも同じです。全画面のレールでは、キャンバスが高くなるのに合わせてカメラも引くので、画面上の大きさはそのままに、立ち位置が上がって脚も膝下まで見えます。下端の切れ目はグラデーションでページにやわらかく溶けます。`,
