...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-08-11',
    title: `アシスタントが全画面で開き、資料を探す過程も見せるようになりました`,
    tags: ['feature', 'design'],
    body: [
      `このサイトの右下にある AI アシスタントは幅 400px しかなく、回答ひとつと引用元だけで画面が埋まっていました。今回、全画面表示を追加し、増えた場所を小さいパネルには入りきらなかったことに使っています。資料を探す各ステップを、左のレールでそのまま動かして見せます。`,
      { kind: 'heading', text: `3 つのサイズ、閉じるボタンはなし` },
      `状態は 3 つになりました。右下のピル、これまでの小さいパネル、そしてページ全体を覆う全画面です。どれからどれへも直接切り替えられ、Esc を押すと 1 段ずつ戻り、ピルから開き直すと前回使っていたサイズに戻ります。`,
      `「閉じる」はあえて作っていません。パネルをしまっても会話が消えたことは一度もなく、それを閉じると呼ぶのは嘘になるので、「最小化」に変えました。本当に消したいときは、メッセージの下にあるクリアを押してください。`,
      { kind: 'heading', text: `左のレールがしていること` },
      `ひとつの質問に答える裏では、いくつもの工程が動いています。まずこのメッセージがどの経路を通るか判断し、質問をベクトルに変えて検索し、返ってきた内容に点数をつけ、結果が弱ければ質問を書き換えてもう一度検索し、最後に回答を書く。以前はこれ全部が「検索中⋯⋯」の一言の裏に隠れていました。`,
      `レールはその一段ずつを並べます。進んだところまで点灯し、そのステップが終わると何ミリ秒かかったかを横に表示し、上部で合計時間を足していきます。ステップの間には光がコネクタを伝って流れ、仕事が次に引き渡されたことが見えます。`,
      { kind: 'heading', text: `作っている途中で、流れが直線ではないと分かった` },
      `最初のデザイン案はノードを 5 つ一列に並べたものでしたが、実データにつないで間違いだと分かりました。雑談やよくある質問と判定された問いは最初のステップでそのまま答えて終わり、検索には一切触れません。結果が弱かった問いでは、システムが自分で質問を書き換えてもう一度検索するので、同じステップが 2 回現れます。`,
      `そのためレールは、固定の 5 マスではなく今回実際に走った内容を描きます。やり直した検索は独立した 1 station として並びます。自分の結果が弱いと気づいて、もう一度やり直す。この検索で一番見どころのある動きだからです。`,
      { kind: 'heading', text: `細かい調整` },
      {
        kind: 'list',
        items: [
          `スマートフォンの全画面は角丸なしで画面いっぱいに広がり、レールは畳まれます。理由は地味です。390px の画面でデスクトップと同じ 16px の余白を残すと、全画面は小さいパネルより 8px 広いだけになり、何も起きていないように見えます。`,
          `システム設定でモーションを減らしている場合はすべてのアニメーションが止まり、ステップ名と所要時間はそのまま残ります。情報が動きの中だけに存在してはいけません。`,
          `接続が切れたり無料枠を使い切ったりした場合、実行中だったステップは「中断」と表示されます。開発中の一時期は永久に回り続けていました。うまくいく経路しか処理していなかったからです。`,
        ],
      },
    ],
  },
