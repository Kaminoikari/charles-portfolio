...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      'サイト隅の「このポートフォリオに質問」ボタンを押すと、私が一から設計・構築したチャットボットが開きます。英語・中国語・日本語で私の仕事について答え、私の実際のポートフォリオデータのみに基づくので、作り話はしません。ChatGPT の薄いラッパーではなく、実際に動く AI エンジニアリング作品として作りました。',
      { kind: 'heading', text: '質問にどう答えるか' },
      '使っているのは RAG（検索拡張生成）です。言語モデルが「覚えている」ことに頼るのではなく、まずポートフォリオから関連する事実を探し、それに基づいて答えを書きます。私のものはさらに「自己修正する」RAG で、自分の回答をチェックします。',
      {
        kind: 'list',
        items: [
      ...
      '公開チャットボットは「プロンプトインジェクション」（ルールを無視させたり有害な発言をさせたりする手口）の標的です。3 層で防御します。',
      {
        ...
        items: [
          ...
          '**スコープの固定**：モデルにはすべてのメッセージを「指示」ではなく「データ」として扱い、私に関する本当の質問でなければ：数学や言葉遊びに偽装されていても：拒否するよう指示しています。',
