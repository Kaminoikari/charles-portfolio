...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-08-21',
    title: `ミカが中国語を話すようになり、英語もカタカナ表記をやめました`,
    tags: ['feature', 'technical'],
    body: [
      `彼女の中国語音声は、これまで存在していませんでした。繁体字サイトの訪問者が聞いていたのは翻訳されていない日本語クリップで、英語サイトではカタカナで綴った英語でした。VOICEVOX には日本語以外の音素がないため、それが唯一できることだったからです。今は三つのサイトがそれぞれ 25 句の録音を持ち、しかも同じ声です。`,
      { kind: 'heading', text: `最初に試した順当な方法は、耳で否決されました` },
      `第一版は彼女の声を多言語モデルにクローンし、そのまま中国語を話させるものでした。中国語には日本語なまりが残り、英語は聞き取れないほどでした。設定の問題ではありません。クローンの参照音声はすべて日本語なので、モデルには「この話者が中国語や英語をどう発音するか」の証拠がなく、日本語の音の作り方を流用するしかありません。切り分けのテストで確認済みです。同じモデルがクローンを着けなければ、ごく普通の中国語を話します。`,
      { kind: 'heading', text: `なまりはなまりのある人から、声色は彼女から` },
      `採用した方法は、話された一文を二つに分けて調達します。まず台湾なまりのネイティブ音声が台詞を読み、発音と抑揚がここで決まります。声色は他人のままです。次に voice conversion がフレーム単位で声色だけを彼女のものに書き換え、発音済みの音には触れません。この二つを切り離せる操作は、これだけです。`,
      {
        kind: 'table',
      ...
      { kind: 'heading', text: `リップシンクは道連れになりませんでした` },
      `彼女の口は事前に焼き込んだタイムラインで動いており、その元データは VOICEVOX が合成時に報告する母音ごとの時間情報でした。合成器を替えれば失われるはずのものです。ところが新しい合成器も同じ情報を返します。中国語は音節ごと、英語は単語ごとの開始と終了です。さらに voice conversion は長さを変えず、これはクリップごとに実測しました。したがって変換前に取った時間は、変換後も同じ音節を指しています。`,
      `ただし時間情報が示しているのはテキストであり、テキストと音声は両端でずれます。全体の四分の一のクリップでは、合成器が「言い終えた」と申告した最後の文字のあとも最大 415 ミリ秒鳴り続けており、二句は冒頭が母音を持たない鼻音（英語の Mm、中国語の嗯）で、口が動き出すまで 0.5 秒近くかかっていました。そこで各クリップの最初と最後の口形は、音声そのものから計測した開始点と終了点に固定しています。50 本すべてが、実際に声が出ている瞬間から 1 ミリ秒以内で開閉するようになりました。`,
      {
        kind: 'stats',
        items: [
          { value: '50', label: `新規録音` },
      ...
      `笑い声はそのままです。頭をなでたときの三つのえへへには言語が含まれないので、三サイトとも同じファイルを再生します。例外は三回続けてなでたときで、文句は言葉なので、三つの言語それぞれに一句あります。`,
    ],
  },
  {
    id: 'mika-motion-capture',
    date: '2026-08-19',
    title: `ミカのしぐさがモーションキャプチャーになり、すべてのポーズが公開前に計測される`,
    tags: ['feature', 'technical'],
    body: [
      `彼女の腕のしぐさは、手で打ち込んだボーンの角度でした。実際の骨格に当てて計測したところ、10 個のうち 7 個は名前が語る場所に手を置いていませんでした。「髪に触れる」と名づけられたしぐさで手は腰に止まり、頬をつつくしぐさでは指先が顔の内側に入り、ピースサインでは両方の手のひらが見る人に背を向けていました。10 個すべてを外しました。いま彼女が演じるのは VRoid Project のモーションキャプチャーで、新しいテストがクリップの手の位置を実測してから公開を許します。`,
      { kind: 'heading', text: `何が壊れていたのか` },
      `あのしぐさたちが持っていた自動チェックは、腕の横方向のリーチがキャンバス幅に収まるか、それだけでした。手がどこに落ちるか、手のひらがどちらを向くか、指が顔を貫通しないか。どれもテストからは見えず、調整はスクリーンショットの目視に頼るしかありません。あるコマで正しく見えた修正が、別のコマを静かに壊していました。`,
      {
        kind: 'table',
      ...
      { kind: 'heading', text: `モーションキャプチャー、そして検査を通ったものだけ` },
      `いま彼女が演じるのは VRoid Project 公式アニメーションの 3 本です。顔の横で構えるピースサイン、静かな立ちポーズ、その場で一回転。関節が噛み合っているのは、実際に人がそう動いたからです。同じ公式パックの残り 4 本はテストに弾かれて床に置いたままです。ここが肝心で、公式アセットであっても計測を通り抜ける必要があります。`,
      {
        kind: 'list',
        items: [
          '`greeting` は腰が立位より 0.57m 低い位置から始まり、最初の 4 秒で床から浮き上がってきます。6 秒あたりで指先が頬を通り抜けます。',
          '`shoot` はあと一歩ですり抜けるところだった 1 本です。見る人に向けて上げるフィンガーガンはきれいに見え、検査を通って公開待ちに入っていました。サンプリングを人差し指の先から手の 16 関節すべてに広げるまでは。親指が 4 分の 1 秒ほど頬に約 5mm 食い込みます。人差し指だけなら 1.19 で合格、手全体なら 0.90 で不合格です。',
          '`showFullBody` は横に 0.713m 伸びてキャンバスそのものを越え、そして一度も手のひらを見る人に向けません。',
          '`squat` は腰を 0.218m 落とし、画面の下端近くまで沈みます。',
        ],
      },
      { kind: 'heading', text: `彼女を計測するテスト` },
      `新しい検証層はモデルファイルから骨格を組み直し、関節の計算を Node で走らせます。ブラウザも WebGL も使わないので、全クリップの全キーフレームの計測が 1 秒ほどで終わります。成立すべき条件は 6 つです。両手のどの関節も顔の体積に入らないこと、シルエット全体が実際に見えている画角の内側に留まること、手が主役のクリップはどこかで手のひらを見る人に向けること、腰が立位の高さから 0.08m 以内に留まること、最初と最後は自分の足で立っていること、そして骨格そのものが直立した身体として復元できること。`,
      `最後の 1 つは、読み取り側の不具合を捕まえたので存在します。3 本のうち 1 本はボーンの rest 姿勢の向きが異なり、素直に読むと足が頭の上に来る逆さまの姿勢になります。そしてその姿勢では、他の検査がすべて通ってしまいました。二つ折りの身体は細く低く、手のひらはどこも向かないからです。どの検査も、信頼する前に一度わざと失敗させてあります。しかも赤くするのに必要な操作はそれぞれ違います。弾いたクリップを戻せば多くはそれぞれ対応する欠陥で赤くなり、直立の検査はデコード処理を外したときにだけ赤くなります。公開されるものはテスト結果そのものです。`,
      {
        kind: 'stats',
        items: [
      ...
      `静かなしぐさはそのままです。首をかしげる、あたりを見回す、重心を移す、小さく跳ねる、腰をひねる、床を覗き込む、そして頭を撫でられたときの嬉しそうな揺れ。どれも頭と背骨だけを動かすので、もともと間違えようのある腕がありません。`,
    ],
  },
  {
    id: 'mika-full-height',
    date: '2026-08-14',
    title: `ミカが、いま交わしている会話と同じ高さに立つ`,
    tags: ['design', 'technical'],
    body: [
      `ミカは 560px のパネルの隣に立つ 342px の人物でした。話し相手のガイドというより、隅に置かれた飾りに見える大きさです。いまはチャットのどちらのサイズでも高さいっぱいに立ちます。パネル表示では隣のパネルと同じ高さ、全画面では画面の高さそのものです。`,
      { kind: 'heading', text: `パネル表示：パネルと同じ高さに` },
      `キャンバスはパネル自身の高さになり、幅も同じ比率で追随するので、腕に必要な余白は拡大後も残ります。構図は腰から上のまま、**1.64 倍**です。パネルとキャンバスはコード上で同じ数値を読むため、パネルだけを高くすることはできません。`,
      { kind: 'heading', text: `全画面：彼女専用の一列` },
      `全画面ではこれまで 236px の検索パイプラインのレールの足元に立っていました。高さは 400px で頭打ちになり、拡大したばかりの小さいパネルより全画面のほうが小さいという状態でした。いまは対話の右側に移り、ヘッダーから下のパネル全体の高さを使います。`,
      `テキストを押しのけるのは身体だけです。キャンバスの残りはジェスチャー用の透明な余白で、空いた隙間の上に浮いています。おかげで **132px** の読み幅が残りました。キャンバス全体に合わせて列を切っていれば失っていた分です。`,
      `カメラは高くなったフレームに合わせて構図を取り直しました。以前の構図は髪の上に余白を残していて、400px のキャンバスでは気づきませんが、800px では穴に見えます。上端は頭のすぐ上まで下り、下端は膝のままなので、同じキャンバスで **10% 大きく**映ります。`,
      { kind: 'heading', text: `ウィンドウが狭いときは、動かさずに小さくする` },
      `どの幅でも別の場所へ移ることはありません。サイズは「パネルの高さ」と「ウィンドウ幅が払える高さ」の小さいほうを比率固定で採るので、狭いウィンドウでは同じ位置に一回り小さいミカが立ち、ジェスチャーはすべてフレームに収まったままです。低いウィンドウでは完全に復活しました。高さ 640px を下回ると消えていたのは、彼女がパイプラインの上に重なるからというだけの理由で、自分の列に立つ以上、重なる相手がいません。`,
      { kind: 'heading', text: `おすすめの質問は出したまま` },
      `レールは質問を送った瞬間におすすめの質問をパイプラインに差し替えていました。これはおすすめの質問の問題ではありません。彼女がレールの足元に立っていたため、両方を積むと最後のステーションが頭の後ろに隠れたのです。いまは両方がレールを共有してスクロールするので、続けて質問するのに回答を待つ必要はありません。`,
    ],
  },
  {
