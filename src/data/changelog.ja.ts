...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      `右下の AI アシスタントに顔と身体がつきました。全身の **3D キャラクター**そのものが起動ボタンで、スマートフォンでもデスクトップでも表示されます。会話の状態にリアルタイムで反応します。何もないときは辺りを見回し、入力を始めた瞬間から下書きを追うように顔を上下に動かし、回答がストリーミングされる間は母音の口形で口が動き、全身の色調がサイトの mars orange へ暖かく変わって、回答が終わるとゆっくり元に戻ります。スマートフォンより広いウィンドウの全画面モードでは検索パイプラインのレールの下に立ち、ステーションが順に点灯するのを横目に回答を「読み上げ」ます。`,
