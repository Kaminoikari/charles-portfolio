...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-08-14',
    title: `Mika 進化：フレーム精度のリップシンク、生きた表情、モデル 64% 軽量化`,
    tags: ['feature', 'design', 'technical'],
    body: [
      `デビューの翌日、**Mika** は演技を覚えました。口の動きは音節ごとに音声へぴったり合い、顔には本物の感情がのり、手を振って挨拶し、お辞儀で見送ります。カメラも寄せて、その細部がちゃんと見える距離になりました。あわせてダウンロードサイズも **64% 軽く**なりました。`,
      { kind: 'heading', text: `合成エンジン自身のタイミングによるリップシンク` },
      `各ボイスクリップには、VOICEVOX が合成時に報告する母音ごとの正確な時間情報から焼き込んだ口形タイムラインが付属します。再生中はエンジンが毎フレーム audio 要素自身の時計を読み、その瞬間の母音口形を引き当て、5 つの口形チャンネルがクロスフェードします。結果はフレーム精度のリップシンクで、ランタイムの音声解析はゼロ。ブラウザは音声信号に一切触れないため、コストはほぼゼロで、音声が鳴る環境ならどこでも動きます。`,
      { kind: 'heading', text: `演技する顔と身体` },
      {
        kind: 'list',
        items: [
          `話す場面ごとに表情がつきます。挨拶とお別れは happy、質問を受け取るときはリラックス、エラー時は申し訳なさそうに。表情はなめらかに入り、クリップより一拍長く保ち、ゆっくり抜けます。`,
          `ジェスチャーも語彙に加わりました。挨拶で手を振り、質問にうなずき、チャットを閉じるとお辞儀します。`,
          `合間には呼吸し、ゆっくり重心を移し、視線を小さく走らせ、ときどき二連まばたきするので、マネキンのように固まる瞬間がありません。`,
          `3 つの状態はそれぞれ固有の視線リズムを持ち、その間をフィルタがつなぐようになりました。回答を終えてアイドルに戻るとき、頭は約 0.4 秒かけて向き直ります。`,
        ],
      },
      { kind: 'heading', text: `シネマティックなレンダリングと登場演出` },
      `レンダリングはフィルミック（ACES）トーンマッピングと高めのピクセル密度に移行。回答中はマーズオレンジのリムライトが強まり、足元にはソフトな接地影、下からのシアンの補助光が暗い衣装を暗いページから引き離します。登場にも演出がつきました。シアンの粒子が立ちのぼり、短い発光と小さなスケールポップとともに実体化します。`,
      { kind: 'heading', text: `表情が読める距離まで` },
      `この演技はこれまで、高さ約 27px の顔の上で起きていました。スマートフォンでは約 19px です。カメラは腰から上を切り取る構図になり、顔は **1.7 倍**に。ページ上で彼女が占める面積はそのままなので、ほかの要素は動かす必要がありませんでした。チャットを開くとさらに大きくなり、キャンバス自体が広がってネイティブ解像度で描かれます。パネル表示でも全画面でも同じです。全画面のレールでは、キャンバスが高くなるのに合わせてカメラも引くので、画面上の大きさはそのままに、立ち位置が上がって脚も膝下まで見えます。下端の切れ目はグラデーションでページにやわらかく溶けます。フレームは腕の長さにも合わせました。背伸びの指先はキャンバスの端で切れていたので、3 つの配置それぞれに横方向の余白を足しています。視野角は垂直基準なので、この余白は見た目の大きさを一切変えません。`,
      { kind: 'heading', text: `頭をなでる` },
      `頭の上を何度か往復してなでると、うれしそうに小さく頭を揺らします（静かに。声を出すのは実際にタップしたときだけ）。放っておくと、およそ 5 秒ごとに 11 種類の小さなしぐさからランダムにひとつ披露します。伸び、首かしげ、あたりを見回す、手のひらを眺める、重心移動、小さなバウンス、腕の揺らし、髪に触れる、深呼吸、腰ひねり、足元をのぞき込む、のどれかです。`,
      { kind: 'heading', text: `64% 軽く、失ったものはゼロ` },
      `モデルのテクスチャを WebP に再パックし、VRM の全機能はそのまま。スケルトン・表情・スプリングボーンのデータはバイト単位で一致を検証済みです。ダウンロードは 15.4MB から 5.5MB（実転送は約 1.9MB）へ。画面上のキャラクターは 1 ピクセルも変わっていません。`,
    ],
  },
  {
    id: 'avatar-guide-3d',
    date: '2026-08-13',
    title: `AI アシスタントに全身 3D キャラクターが登場`,
    tags: ['feature', 'design', 'technical'],
    body: [
      `右下の AI アシスタントに顔と身体、そして名前がつきました。**ミカ（Mika）**、全身の **3D キャラクター**そのものが起動ボタンで、スマートフォンでもデスクトップでも表示されます。会話の状態にリアルタイムで反応します。何もないときは辺りを見回し、入力を始めた瞬間から下書きを追うように顔を上下に動かし、回答がストリーミングされる間は母音の口形で口が動き、全身の色調がサイトの mars orange へ暖かく変わって、回答が終わるとゆっくり元に戻ります。スマートフォンより広いウィンドウの全画面モードでは検索パイプラインのレールの下に立ち、ステーションが順に点灯するのを横目に回答を「読み上げ」ます。`,
      { kind: 'heading', text: `チャット状態で駆動する VRM ヒューマノイド` },
      `キャラクターは **VRM** モデル（VTuber ツール群で共通のヒューマノイド形式）で、three.js と \`@pixiv/three-vrm\` で描画しています。頭の動きはボーンの回転です。頭と首、わずかに背骨まで一緒に回り、目は注視ターゲットを追うので、人が振り向くのと同じ見え方になります。発話は日本語の母音 5 つの口形を回すループで、テンポはあえて不均一にしています。等間隔の開閉は機械に見えるからです。`,
      `これらすべてを、チャットウィジェットに元からあった 2 つの信号だけで駆動しています。入力欄に文字があるか、回答がストリーミング中か。カーソル追跡も音声解析もありません。`,
      { kind: 'heading', text: `コストと、免除される訪問者` },
      {
        kind: 'list',
        items: [
          `モデルは 15.4MB。lazy ロードで、ヒーローのオープニング演出が終わってから取得を始め、帯域を演出に譲ります。再訪はブラウザキャッシュが吸収します。`,
          `**prefers-reduced-motion** を有効にしている訪問者と WebGL2 のないハードウェアでは、以前とまったく同じインターフェース（カプセル型の起動ボタン）のままです。WebGL2 の検出は最後に回してあり、この訪問者は graphics context のコストを一切払いません。`,
          `キャラクター自身が起動ボタンです。aria-label とキーボード操作はそのまま引き継ぎ、focus ring は足元のグローリングに置き換えました。初回訪問時には吹き出しが 8 秒だけタップを誘い、その後は姿を消します。対象外の訪問者や、読み込みが失敗・長引いた場合は従来のカプセル型ボタンが代役を務めます。正常な読み込み中は、彼女が現れるまで隅は空けたままです。`,
          `スマートフォンではキャラクター自身が隅に立って起動ボタンを務め、チャットパネルが狭い画面を覆っている間は完全に退場して描画も止まります。`,
          `声もつきました。24 の日本語ショートフレーズ（VOICEVOX:春日部つむぎ、クレジットはフッターに記載）が 8 つの瞬間をカバーします。初回オープンの自己紹介、タップへの挨拶、質問送信への相づち、フルスクリーン移行、おすすめ質問の選択、クローズ時のお別れ、回答完了の締め、エラー時の一言です。話しているあいだは口も動きます（回答時と同じ口形ループで、音声解析はしません）。音声は最初のタップからそのまま鳴ります（サイト全体の BGM スイッチは同時に廃止しました）。再生されるのは訪問者自身のタップの中だけで、勝手に鳴り出すことはありません。英語版サイトでは同じ声がカタカナ英語で話します。日本語なまりは意図的なチャームポイントです。訪問して最初にチャットを開いたときだけフルの自己紹介をして、その後は短いフレーズに戻ります。`,
        ],
      },
      `現在のキャラクターは VRoid Studio 公式のサンプルモデルで、自作キャラクターが完成するまでの代役です。差し替えはファイル 1 つの置き換えで済みます。`,
    ],
  },
  {
    id: 'ambient-music-opt-in',
    date: '2026-08-13',
    title: `アンビエント BGM の自動再生を廃止`,
    tags: ['design'],
    body: [
      `これまでは、Enter をクリックした訪問者に限り、オープニング演出が終わった瞬間に BGM が自動でフェードインしていました。テスト中は気の利いた演出に思えましたが、実際には訪問者の代わりに決めてしまう挙動でした。サイトを見たいだけの人に、頼んでいない音を届けていたのです。`,
      `音楽は完全にオプトインになりました。左下のスピーカーボタンが唯一のスイッチで、アイコンは常に再生状態をそのまま示します。スピーカーのタップ自体が iOS Safari の「音は実際のジェスチャー内でしか始められない」という規則を満たすため、どのページでも、オープニング演出を見ていなくても、最初のタップで音楽が流れます。副次的な利点として、音を求めていない訪問者のために無音のストリームを流し続けることもなくなりました。`,
    ],
  },
  {
    id: 'chatbot-fullscreen-pipeline',
    date: '2026-08-11',
    title: `アシスタントに全画面モードを追加し、RAG 検索パイプラインをその場で可視化`,
    tags: ['feature', 'design'],
    body: [
      `このサイトの右下にある AI アシスタントは幅 400px 固定で、回答ひとつと引用元だけでパネルが埋まり、長い回答を小さな枠でスクロールする必要がありました。今回、全画面モードを追加し、広くなった領域を小さいパネルには入りきらなかったものに充てています。アシスタントの裏側で動く **RAG 検索パイプライン**（retrieval-augmented generation、資料を検索してから回答を書く仕組み）を、左のレールにその場で描きます。このパイプラインはアシスタントで最も技術的に面白い部分でありながら、これまで「検索中⋯⋯」の一言の裏に収まっていました。`,
      { kind: 'heading', text: `3 つのウィンドウ状態、閉じるボタンは置かない` },
      `状態は 3 つです。右下のピル、これまでのドッキングパネル、ページ全体を覆う全画面。どの状態からどの状態へも直接切り替えられ、Esc は 1 段ずつ戻し、ピルから開き直すと前回使っていたサイズに戻ります。`,
      `「閉じる」はあえて外しました。パネルをしまっても会話が消えたことは一度もなく、そのボタンを「閉じる」と呼ぶと履歴が消えたと受け取られます。ラベルと実際の挙動が食い違っている状態です。今は「最小化」で、本当に消すためのクリアはメッセージの下に独立して置いてあります。`,
      { kind: 'heading', text: `左のレールが表示しているもの` },
      `ひとつの質問に答える裏では、いくつかのノードが順に動きます。**triage** がこのメッセージの経路を判断し、質問をベクトル化して検索し、返ってきた文書に点数をつけ、点数が低ければクエリを書き換えて再検索し、最後に回答を生成します。この「自分の結果が弱ければやり直す」設計を **corrective RAG** と呼びます。`,
      `レールはそのノードを 1 station ずつ並べます。実行が進んだところまで点灯し、ノードが終わるたびに要した時間をミリ秒で横に表示し、上部で合計を加算します。station の間では光がコネクタを伝って流れ、処理が次に引き渡されたことが見えます。`,
      { kind: 'heading', text: `実際のパイプラインは分岐し、自分でやり直す` },
      `最初のデザイン案はノードを 5 つ一列に並べたものでしたが、実データにつないだ時点でトポロジーが食い違っていると分かりました。雑談や FAQ キャッシュのヒットと triage で判定された質問は、その場で答えて終わり、検索には到達しません。返ってきた文書が弱い質問では、システムがクエリを書き換えて再検索するため、同じノードが 2 回現れます。`,
      `そこでレールは、今回実際に走ったノードの並びを描きます。やり直した検索は独立した 1 station として並びます。自分の結果が弱いと気づいてもう一度走る、この動きが corrective RAG の見どころだからです。`,
      { kind: 'heading', text: `いくつかの実装上のトレードオフ` },
      {
        kind: 'list',
        items: [
          `スマートフォンの全画面は角丸なしで画面いっぱいに広がり、レールは畳まれます。390px の画面でデスクトップと同じ 16px の余白を残すと、全画面はドッキングパネルより 8px 広いだけになり、ボタンが効いていないように受け取られます。`,
          `**prefers-reduced-motion** が有効な場合はすべてのアニメーションを停止し、ノード名と所要時間はそのまま表示します。情報がアニメーションの中だけに存在してはいけません。`,
          `接続が切れたり無料枠を使い切ったりした場合、実行中だったノードは「中断」と表示します。開発中は永久に回り続けていました。成功する経路しか処理していなかったからです。`,
        ],
      },
    ],
  },
  {
    ...
    date: '2026-07-31',
    title: `アシスタントが文脈を覚え、まとめて聞いた複数の質問にも答えられるように`,
    tags: ['feature', 'technical'],
    body: [
      `これまでこのサイトの AI アシスタントは、メッセージを一通受け取るたびに独立した一問として処理していました。「USPACE で何をしたの？」と聞いた後に「じゃあチームの規模は？」と続けても、「チーム」がどの話を指すのか分からず、また一から推測するだけでした。今回は二つを追加しました。**会話メモリ**で追加質問を受け止められるようにし、**質問の分解**で、一通に複数の質問が入っていてもそれぞれ別々に検索してからまとめて回答できるようにしました。`,
      { kind: 'heading', text: `二つの弱点：追加質問の取りこぼしと、検索の希薄化` },
      `問題は二種類ありました。追加質問では、代名詞や指示語（「彼」「あのプロジェクト」「二つ目」）が前のターンを指した瞬間、単一質問の検索は誰を探せばいいのか分からなくなります。複数質問では、メッセージ全体が一つのクエリとして検索に入り、決まった件数の枠が複数のトピックで分け合われ、弱いトピックが押し出されて答えが一部欠けます。`,
      { kind: 'heading', text: `どう受け止めているか` },
      `会話メモリは一般的な condense-question の手法です。フロントエンドが直近の数ターンを送り、検索に入る前に安価なモデル呼び出しを一度だけ行って、「じゃあチームの規模は？」を「USPACE で Charles が率いたチームの規模は？」のように単独で意味の通る質問へ書き換えます。ポイントは履歴を使うのがこの一箇所だけということで、後続の検索、FAQ 照合、生成には自己完結した一つの質問が渡るので、いちばんコストのかかる生成プロンプトが履歴のせいで膨らむことはありません。履歴のない初回はこの工程ごとスキップし、コストは従来と同じです。`,
      `質問の分解は、まずごく安価な判定を通します。一通に疑問符が二つ以上、または明示的な番号付けがある場合だけ、無料枠の呼び出しを一度使って独立した小質問に分けます。分解後は小質問ごとに検索を一度ずつ走らせ、結果を交互に取り混ぜて統合するので、どの質問も自分に最も関連する断片を得られます。同時に統合後の総量を上限（現在は小質問は最大四つ、保持は八件まで）に抑えるため、生成プロンプトが膨らむこともありません。単一質問（つまり大多数のトラフィック）は文字列判定すら通り抜け、追加のモデル呼び出しは発生しません。`,
      { kind: 'heading', text: `なぜこの折衷を選んだか` },
      `ここにはトレードオフがあります。複数質問に対するフロンティアラボの定石は、モデル自身に質問を分けさせ、質問ごとに検索して統合させるやり方で、品質は最も高い一方、小質問ごとに検索が一巡し、トークンと呼び出し回数が増えます。このアシスタントはコスト優先の設計で、FAQ キャッシュと Gemini の無料枠で大半を吸収し、そもそもトラフィックも少なめです。そこで常に分解するのではなく、フロンティアの分解手法の前にゲートを置き、本当に必要なときだけそのコストを払います。通常の単一質問は従来どおりの価格で、追加質問と複数質問だけが少し多く払う形です。`,
      { kind: 'heading', text: `会話メモリ：なぜ履歴をそのまま入れないのか` },
      {
        kind: 'table',
        columns: [`方式`, `検索を直す？`, `1ターンのトークン`, `判断`],
        rows: [
          [`メッセージ全体を一問として扱う（以前）`, `いいえ`, `最小`, `追加質問が崩れる`],
          [`履歴をそのままプロンプトに入れる`, `いいえ（言い回しだけ）`, `高い；毎ターン膨らむ`, `不採用`],
          [`Query contextualization`, `はい`, `ごく僅か；追加質問時に小さな呼び出し一度`, `**採用**`],
        ],
      },
      { kind: 'heading', text: `複数質問：なぜ毎回分解しないのか` },
      {
        kind: 'table',
        columns: [`方式`, `質問ごとに検索？`, `単一質問のコスト`, `判断`],
        rows: [
          [`一括で一度だけ検索（以前）`, `いいえ；枠が分割される`, `最小`, `複数質問に弱い`],
          [`常に先に分解`, `はい`, `高め；単一質問も余分に払う`, `不採用`],
          [`ゲート付き分解`, `はい（複数検出時のみ）`, `追加ゼロ`, `**採用**`],
        ],
      },
      `どの経路にもフォールバックがあります。ゲートの見落とし、分解の失敗、あるいは何も検索できなかった小質問は、いずれも元の単一質問フローに戻るので、新機能が回答全体を壊すことはありません。分析データも変えていません。チャットログには、書き換え後ではなく訪問者が実際に打った文言がそのまま残ります。`,
    ],
  },
  {
  ...
  },
  {
    ...
    tags: ['feature', 'design'],
    body: [
      'ADPList のメンターになったので、ホームページから直接わたしの時間を予約できるようになりました。新しい Mentorship セクションを記事とコンタクトフッターの間に配置し、ナビゲーションとフッターもそこへ誘導するよう更新しました。',
      { kind: 'heading', text: '変更点' },
      {
        kind: 'list',
        items: [
          '**Mentorship セクション**：ダークなテーマになじむ call-to-action カードで、必要なときだけ ADPList のライブ予約ウィジェットをその場で読み込み、ページを離れずに空き枠を選べます。',
          '**ナビゲーションのアンカー**：デスクトップとモバイルのメニューに MENTORSHIP リンクを追加し、クリックすると新セクションへスムーズにスクロールします。',
          '**フッターのプロフィールアイコン**：公式の ADPList マークをソーシャル列に追加し、カラーマスクで描画することで、ほかのアイコンと同じ落ち着いたグレーとマースオレンジのホバーに揃えました。',
          '**より即時なフッターのホバー**：ソーシャルアイコンはこれまで登場時のスタッガー遅延をホバーにも引き継いでいて、色の変化がカーソルに遅れていました。いまはホバーの反応が即座に発火し、登場時のスタッガーはそのまま保たれます。',
        ],
      },
    ],
  },
    ...
    body: [
      ...
      { kind: 'heading', text: '変更点' },
      {
        ...
        items: [
          ...
          '**ポートフォリオの内容を優先**：検索ではサイト自身のキュレーションされた情報（プロフィール、プロジェクト、経歴）に外部記事より高い重みを与え、ブログの余談が一次情報を埋もれさせないようにしました。',
        ],
      },
    ],
  },
    ...
    body: [
      ...
      { kind: 'heading', text: 'どう描画しているか' },
      {
        ...
        items: [
          ...
          '**タップで始まる音声**：Enter スプラッシュがあるのは、入ることを選んだ瞬間にオープニング音を鳴らすためです。ブラウザはユーザー操作の後でしか音を再生しないからです。',
        ],
      },
    ],
  },
