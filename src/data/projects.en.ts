...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    problem: [
      'Hunting for a property on 591 traps users in a low-efficiency scan → evaluate → discard loop. Listings are highly volatile and noisy: repeat postings, price drift, fragmented data across platforms. Users end up juggling dozens of tabs to manually cross-reference MRT proximity, school districts, layouts, and agent reputation. For time-constrained working professionals, the cost of decision-making and the sheer volume of noise routinely bury high-quality listings. Existing platforms expose only basic field filters; they lack any synthesized diagnosis of listing context (rent against district median, spatial ratios, tenancy risk), forcing users to repeat low-value information synthesis every single day.',
    ],
    solution: [
      'This project ships a Node.js (ESM) automation pipeline driven by macOS launchd. At 09:00 daily, agent-browser scans the configured 591 search areas and deduplicates against persistent history (JSON / TSV), emitting a state delta of new, price-dropped, and delisted entries. Each listing runs through a five-dimension heuristic score (price, space, location, condition, risk) with weights tuned per scenario (rental vs. purchase). Listings scoring ≥ 4.0 are flagged as priority recommendations and rendered into a visual HTML digest delivered via Nodemailer. The system also integrates Claude Code as an interactive decision layer for real-time affordability simulations, side-by-side comparisons, and standardized visit-prep checklists: the pipeline owns the data funnel, the AI handles the trade-off judgments.',
    ],
    techStack: [
      { category: 'Runtime', items: 'Node.js (ESM, .mjs)' },
      { category: 'Scraping', items: 'agent-browser (591 data collection)' },
      { category: 'Email', items: 'Nodemailer + Gmail SMTP' },
      { category: 'Scheduling', items: 'macOS launchd (system-level automation)' },
      ...
      { category: 'Source', items: '591.com.tw (rental / purchase)' },
    ],
    impact: [
      'Scheduled scanning: triggers the pipeline at 09:00 daily, with persistent caching for precise deduplication.',
      'Five-dimension scoring: a quantitative model with scenario-tuned weights (e.g., 30/20/25/15/10 for rentals) that converts subjective impressions into data signals.',
      'Daily email digest: structured HTML reports delivered on schedule, including listing alerts, price-drop tracking, and district breakdowns, optimized for the morning decision window.',
      ...
      'Interactive Claude modes: in-session AI consultation covering financial modeling and deep listing comparison, sharpening last-mile decision quality.',
    ],
    learnings: [
