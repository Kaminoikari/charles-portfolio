...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Job Ops',
    description:
      'Every morning at 7, a Python pipeline grabs the day\'s fresh 104 job listings, scores each one against my CV and personal priorities (comp, remote, growth, team, location), and emails me a sorted shortlist banded RECOMMEND, CAUTIOUS, or SKIP. When a listing is worth deeper research (is this company legit, IC or manager track, how should I prep the interview), seven interactive Claude Code modes pick up that work in conversation.',
    ctaText: 'EXPLORE',
...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    subtitle: 'A personal Python pipeline that does to job listings what HR-side ATS does to candidates: structured scoring, ranked daily, banded into RECOMMEND / CAUTIOUS / SKIP. macOS launchd starts the run at 07:00, fresh 104 listings come in, my CV and personal priorities drive the scoring, and Gmail delivers the digest before 07:30. Seven Claude Code interactive modes handle the slower judgment calls in-session: company legitimacy, level strategy, comp research, interview prep.',
    metaTitle: 'Job Ops — A Candidate-Side ATS for Personal Job Search | Charles Chen Personal Project',
    metaDescription: 'A personal Python pipeline that scores fresh 104 job listings against my CV every morning at 7 and emails a sorted RECOMMEND / CAUTIOUS / SKIP shortlist. Paired with seven Claude Code modes for legitimacy checks, level strategy, comp research, and interview prep. A personal job-search OS by AI Product Manager Charles Chen.',
    problem: [
      'On the HR side of the hiring funnel, ATS does the heavy lifting: a structured rubric scores 100+ candidates daily, ranks them, and trims the tail. On the candidate side, there is no equivalent. Thirty to fifty target listings get read by eye, page by page, with comp, location, team size, and remote policy all juggled in working memory at once. Triage paralysis sets in fast: by the eighth listing the typical exit is to click whatever sits at the top of the recommendation feed. That quietly hands filtering authority straight back to the platform, so their ranking becomes your ranking.',
      'Search filters on 104, LinkedIn, and CakeResume only express hard constraints (salary floor, region, function). They cannot represent weighted preference ("growth > comp > commute"). They cannot match a listing against your specific CV. They cannot shift weights between casting-a-wide-net early in the search and closing-on-an-offer late in it. The candidate ends up doing the matching by hand, at a much smaller batch size than HR-side ATS runs at.',
    ],
    solution: [
      'Job Ops is the candidate-side equivalent of HR\'s ATS: the same structured scoring loop, pointed at the jobs instead of at the candidates. macOS launchd kicks off the Python pipeline at 07:00 daily. Fresh listings come in from 104. Each one runs through a CV-aware evaluator: it reads my markdown CV, matches it against a candidate archetype defined in YAML, scores the role, and bands the result as RECOMMEND, CAUTIOUS, or SKIP. The digest lands in my Gmail inbox before 07:30 as both HTML (the morning glance on my phone) and Markdown (filed to the Obsidian archive for later retrospective).',
      'The eight scoring weights (comp, remote, tech stack, growth, team, brand, location, lifestyle) live in a YAML file, not in Python. Shifting between "casting a wide net" and "closing on an offer" is a one-line edit in config/profile.yml, no code change required. Because that file is under git, every commit on it is a timestamp of how my own priorities shifted across the search. By the end, the git history of one YAML file ends up more informative than any individual daily report when reviewing how the search evolved.',
      'A seven-mode interactive layer takes over for the work the pipeline can\'t reasonably automate, all running inside a Claude Code session: cv-match (single-listing CV alignment), comp-research (market comp survey), legitimacy (company and posting due diligence, backed by community-source lookup), level-strategy (IC vs management track), interview-prep, personalization, and role-summary. The pipeline owns the deterministic work (scrape, dedupe, score, deliver); the interactive modes own the judgment that needs conversation. A TSV scan history keeps lifecycle state (price drops, delistings, repostings) so the same job reposted three times shows up as one row with diff annotations, not three rows of noise.',
    ],
    techStack: [
    ...
    impact: [
      '30 to 50 fresh 104 listings per day collapse into a banded shortlist (RECOMMEND / CAUTIOUS / SKIP) delivered to Gmail before 07:30',
      'Scoring takes the candidate CV (parsed from markdown) and a candidate archetype (defined in YAML), so switching job-search stances ("wide net" vs "closing") is a config edit, not a code change',
      'Seven interactive modes pick up the judgment work the pipeline can\'t automate: company legitimacy against community sources, IC vs manager track, comp negotiation, interview prep',
      'TSV scan history shows price moves and delistings as diff annotations on the same row, so the digest stops being polluted by the same job posted three times in three days',
      'macOS launchd handles scheduling natively; Gmail App Password handles auth without OAuth. Shares the same automation family as house-ops.',
    ],
    learnings: [
      'The engineering pieces (scraper, evaluator, digest) are common to any scoring pipeline. The decision worth defending is who the rubric points at. HR points one rubric at 100 candidates daily; this points one rubric at 100 jobs daily. The build is justified by an asymmetry that bothered me for years: on the candidate side, your only tools for sorting through 30-50 listings are your eyes and your patience. Flipping the rubric is the cheapest fix.',
      'Putting the eight scoring weights in a YAML file (instead of hardcoded in Python) paid off in ways I didn\'t plan for. Because the file is under git, each commit on it is a timestamp of how my own priorities moved during the search. When the search is over, the git log of that single YAML file ends up more informative than any individual daily report. The retrospective writes itself.',
      'The split between a pipeline and interactive Claude Code modes, first tried on house-ops, holds up here too. Deterministic work (scrape, dedupe, score, send email) stays in the pipeline. Conversational work (deciding whether a company is legit, talking through IC vs manager track, role-playing a comp negotiation) goes into interactive modes. Trying to write legitimacy reasoning into a Python module bloats the pipeline beyond recognition; trying to do daily scraping inside a Claude session burns budget and adds fragility. The same architectural cut keeps working across different domains.',
    ],
    links: [
