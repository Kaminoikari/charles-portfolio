...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    body: [
      ...
      `The second is verb form. We decided at first that an explanation should land on polite endings, and wrote her own opening line that way too, which made that line read as a different, politer stranger. All 25 of her recorded lines are plain form, so the rule now is that her two lines are always plain, the explanation between them keeps its polite endings, and the switch falls exactly where she stops talking and the material starts. That line is one a reader can feel.`,
      { kind: 'heading', text: `A written line and a spoken one` },
      `Her lines were short, first person, and passing their tests in the first version, and they still read wrong: the grammar was written prose, where every one of her recordings is speech. "This is my favorite one to answer, because his track record is dense" is an essay. What she says in her recordings is "Nobody knows Charles better than me!" So the standard became the recordings themselves: those 25 lines per language are the only place this character has actually been heard, and text that does not match them is a different character.`,
      `Each language got the marker that separates the two. Chinese lands on a sentence-final particle and reduplicates the way speech does; Japanese uses plain form and opens on an interjection; English contracts everything and stops after one clause. The 164 opening lines and 171 closing lines were rewritten to that standard. The content between them moved in three Japanese paragraphs only, every one of them the 私 to あたし change described above.`,
      `Three more strings never touch a model at all: the speech bubble beside the character, the invitation the panel opens with, and the notice that gets patched into her own message when a request dies. A reviewer found all three. So every string in the panel now carries a label: her talking, a question the visitor is about to ask, or the interface itself. An unlabelled one turns a test red.`,
      { kind: 'heading', text: `Keeping it from breaking again` },
      `Every one of these is pinned by a test now, and we broke each fix again one at a time to confirm those tests actually turn red. The counts the tests quote about themselves are asserted too, so a stale number turns the test suite red and never reaches the next reader.`,
      {
        kind: 'stats',
        items: [
