...
export const changelog: ChangelogEntry[] = [
  {
    ...
    tags: ['design'],
    body: [
      'Rewrote the Plutus Trade case study from a senior PM perspective. The previous draft had two issues: it positioned the product as a consumer SaaS bringing institutional-grade analysis to retail investors — a market frame the live product no longer matches — and the copy leaned on personal/anecdotal voice rather than the analytical framing a recruiter or peer practitioner would expect to read in a case study.',
      'New problem statement frames the user reality in product terms: daily Taiwan equity research is a synthesis bottleneck, not a data availability bottleneck. Monthly revenue normalization, quarterly fundamentals, institutional flow, and K-line technicals are each individually addressable; the cost compounds across a 30–50 name watchlist. Available consumer tooling responds asymmetrically — charting apps surface raw data without interpretation; advisory products interpret but treat the user as passive. Plutus Trade targets the gap in between: domain-literate operators who want AI-assisted synthesis they can override and audit.',
      'Solution section now reads as three load-bearing product decisions rather than feature descriptions: (1) cross-domain AI synthesis with output framed as analysis, not advice; (2) a guided screening flow that translates qualitative investment criteria into an AI-executable contract; (3) an instrumented prediction layer that logs every recommendation with entry context and settles outcomes at horizon, making the system auditable rather than opaque.',
      'Tech stack rewritten from the live repo to correct stale claims: Flutter Web on Vercel (not iOS), FastAPI on Fly.io nrt region, Gemini 2.5 Flash (was 1.5 Pro), Riverpod + go_router + fl_chart + Dio, Pydantic v2 + httpx + APScheduler, three-source FinMind → Yahoo → TWSE/TPEX fallback with seven-day stale cache, Web Push via VAPID. Surfaced the actual feature surface (eight modules: market data center, watchlist and portfolio, AI diagnostics, guided screening, prediction tracking, fundamentals, smart notifications, after-hours daily report).',
      'Learnings rewritten as four PM-voice takeaways covering the load-bearing design decisions: prompt contract design as the first lever before model selection, enforcing the analysis-vs-advice line at the product layer for any financial AI, audience-of-one as a deliberate constraint that frees the design surface to optimize for analytical depth, and data source reliability as a first-class product concern for any decision-support tool — since degradation in this category is functionally an outage of the core value proposition.',
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      'Clicking anywhere on the hero creates a ripple effect that propagates outward through the particles. The ring center stays fixed at screen center rather than following the click, which keeps the composition stable while still feeling responsive.',
