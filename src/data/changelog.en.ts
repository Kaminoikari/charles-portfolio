    ...
    body: [
      ...
      { kind: 'heading', text: `What it costs, and who is excused` },
      {
        ...
        items: [
          ...
          `She also has a voice now: 24 short Japanese lines (voiced with VOICEVOX:春日部つむぎ) cover eight moments, a first-open self-introduction, greeting a tap, acknowledging a typed question, going fullscreen, reacting to a suggested-question pick, saying goodbye on close, wrapping up a finished answer, and owning up to an error, and her mouth moves while she speaks (the same viseme loop the answers use, still no audio analysis). Five more clips are just a laugh, for a pat on the head. Her lines play from the very first tap (the site's background-music toggle was retired at the same time), and every sound she makes is a reply to something you just did, never something she starts on her own. On the English site she speaks the same lines in her own katakana English, same voice, deliberately Japanese-accented. A laugh has no language, so every site plays the same five. The first open of a visit gets a full self-introduction; after that she keeps it short.`,
        ],
      },
