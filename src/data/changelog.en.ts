...
export const changelog: ChangelogEntry[] = [
  {
    ...
    body: [
      ...
      `Each language got the marker that separates the two. Chinese lands on a sentence-final particle and reduplicates the way speech does; Japanese uses plain form and opens on an interjection; English contracts everything and stops after one clause. 164 opening lines and 171 closing lines were rewritten to that standard, and not one word of the content between them moved.`,
      { kind: 'heading', text: `Keeping it from breaking again` },
      `Every one of these is pinned by a test now, and we broke each fix again one at a time to confirm those tests actually turn red. The counts the tests quote about themselves are asserted too, so a stale number fails the build and never reaches the next reader.`,
      {
        kind: 'stats',
        items: [
