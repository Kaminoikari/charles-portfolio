...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      '「AI にシニア PM の頭脳を与え、ひらめきを開発可能な仕様へ変える」。LLM オーケストレーション（Orchestration）ベースの AI Agent：22 種類のプロフェッショナルなプロダクトフレームワーク（JTBD、RICE など）が汎用的な空論を排し、Claude.ai Custom Skill と Claude Code エコシステムへのネイティブ統合で PM の既存ワークフローに直接埋め込まれ、受入条件（AC）付きの開発ドキュメントを生成して真の Dev Handoff を実現します。',
    ctaText: 'EXPLORE',
...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    subtitle: '「AI にシニア PM の頭脳を与え、ひらめきを開発可能な仕様へ変える」。「アイデア」と「開発タスク」の間にある巨大な隔たりを縮めるために設計された、LLM オーケストレーション（Orchestration）ベースの AI Agent：フレームワーク駆動の生成（22 種類のプロフェッショナルなプロダクトフレームワーク（JTBD、RICE など）を内蔵し、漠然とした汎用テキストの生成を拒否）、フルチャネル展開（Claude.ai Custom Skill と Claude Code エコシステムへのネイティブ統合で、PM の既存ワークフローに直接埋め込み）、自動化された技術ハンドオフ（受入条件（AC）付きの開発ドキュメントを生成し、真にシームレスな Dev Handoff を実現）。',
    ...
    problem: [
      '従来のプロダクト開発フローのなかで、品質の高い仕様書（Spec/PRD）を書く作業は、2 つの大きな課題に直面します。',
      '時間コストが極めて高い：合格点の PRD を書くには、ユーザーリサーチ、競合分析、技術制約を統合する必要があり、しばしば数日を要し、開発進行のボトルネックになります。',
      '汎用 AI の限界：ChatGPT や Notion AI はテキストを生成できますが、「プロダクトを理解していません」。JTBD 分析を自動で行うことはできず、RICE モデルで Backlog をどう並べるかも分からない。最終的なアウトプットは、実行に移せない汎用的な空論に終わることが多いのです。',
      'コミュニケーションのギャップ：PM が進捗を急ぐためにドキュメントの整合を飛ばすと、その後の開発と手戻りのコストは何倍にも膨らみます。',
    ],
    solution: [
      'Product Playbook は単なるチャットボットではなく、厳密な仕様生産パイプラインです。',
      'フレームワーク駆動のオーケストレーション：22 個の成熟したプロダクトフレームワーク（Discovery 段階の Positioning から Deliver 段階の PRD まで）を構造化された Prompt に変換します。これにより AI はシニア PM の思考様式で考えることを強制され、出力が実戦的な価値を持つことが保証されます。',
      '柔軟な 6 大モード：異なるプロダクト段階に対して Quick、Full、Revision、Build など 6 種類のモードを提供。スピード重視の機能実験から完全な新規プロダクトのローンチまで、対応する出力深度を見つけられます。',
      '強力なデータ取り込み能力：3 層の PDF パースエンジン（プレーンテキスト → セマンティック認識 → OCR フォールバック）を内蔵し、既存のリサーチ資料をアップロード可能にすることで、AI が空想ではなく事実に基づいて推論できるようにします。',
      '自動変更伝播（Change Propagation）：上流の判断が変わると、下流のドキュメントも同期的に更新されます。さらに重要なのは、CLAUDE.md と TASKS.md を自動生成し、プロダクト要件をエンジニアが理解できる技術タスクへとそのまま翻訳できることです。',
    ],
    techStack: [
      { category: 'Platform', items: 'Claude.ai Custom Skill、Claude Code Plugin/Skill' },
      { category: 'AI Engine', items: 'Claude（LLM Orchestration）、Claude Vision（セマンティックドキュメント解析）' },
      { category: 'Frameworks', items: 'JTBD、Positioning、PR-FAQ、RICE、OST など 22 種類のプロフェッショナルフレームワークを内蔵' },
      ...
      { category: 'Localization', items: '中国語（繁体／簡体）、英語、日本語、韓国語、スペイン語の 6 言語対応' },
    ],
    impact: [
      '効率の最大化：従来数日かかっていた仕様策定プロセスを数分に大幅短縮し、品質はベンチマークを上回る',
      '品質 69% 向上：Skill 未搭載の素のモデルと比べ、プロダクト思考とロジックの厳密さで 69% の向上',
      'ワークフローへのシームレスな融合：マルチチャネル配布により、ユーザーはプラットフォームを切り替えることなく、既存のエディタやターミナル内で呼び出せる',
      'オープンソースコミュニティへの貢献：MIT ライセンスで公開し、PM とエンジニアが共通で使う生産性ツールとして機能',
    ],
    learnings: [
      '「AI の時代において、プロダクトの価値は『フレームワーク』のレイヤーに住んでいる、『生成』のレイヤーではない。」AI PM として、LLM のオーケストレーションが本質的にプロダクトデザインの問題であることを身をもって痛感しました。フレームワークの実行順序は出力品質を直接決めます——例えば Persona を先に定義してから JTBD 分析を実行すれば、AI はより正確に真のユーザーペインを特定できます。',
      'さらに、Skill ベースのチャネル戦略を選んだことが、極めて高い継続利用率をもたらしました。ツールはユーザーに新しいプラットフォームへの適応を強制すべきではなく、ユーザーが本来作業している場所へ「出向いて出会う」べきです。',
      '最後に、私の核心的な気付きはこうです：API は誰でも呼べる、しかし「正しい問いを立て」「思考を導く」方法を知っていることこそが本物の知的財産である、と。この 22 のフレームワークがこのシステムの魂であり、AI はその知恵を高速に届ける配信メカニズムにすぎません。',
    ],
    links: [
