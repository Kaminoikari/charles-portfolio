...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Job Ops',
    description:
      '毎朝 7 時、ある Python パイプラインがその日の新着 104 求人を取得し、履歴書と個人の優先順位（給与、リモート、成長、チーム、勤務地）に照らして 1 件ずつ採点、RECOMMEND / CAUTIOUS / SKIP の 3 段に並べてメールで届けます。深掘りが必要な求人（この会社は実在するのか、IC でいくかマネジメントトラックか、面接の準備をどう組むか）には、7 つの Claude Code インタラクティブモードが対話で引き継ぎます。',
    ctaText: 'EXPLORE',
...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    subtitle: '個人向けの Python パイプライン。HR 側 ATS が候補者に対してしていること（構造化採点、毎日のランキング）を、求人そのものへ向け直したものです。macOS launchd が毎日 07:00 に起動し、104 から当日の新着求人を取得、私の履歴書と個人の優先順位を基に 1 件ずつ採点、RECOMMEND / CAUTIOUS / SKIP の 3 段に分類して、07:30 までに Gmail で手元へ配信します。会話的判断が要る作業は、7 つの Claude Code インタラクティブモードがセッション内で担当：企業の合法性確認、レベル戦略、報酬調査、面接準備。',
    metaTitle: 'Job Ops — 応募者視点の個人版 ATS | Charles Chen 個人プロジェクト',
    metaDescription: '個人向けの Python パイプラインが毎朝 7 時、新着の 104 求人を取得し履歴書に照らして採点、RECOMMEND / CAUTIOUS / SKIP の 3 段でメール配信。企業の合法性確認、レベル戦略、報酬調査、面接準備を担う 7 つの Claude Code モードと組み合わせた、AI プロダクトマネージャー Charles Chen の個人求職 OS。',
    problem: [
      '採用ファネルの HR 側では ATS が重い仕事を担っています。構造化されたルーブリックが毎日 100 人以上の候補者を採点し、ランク付けし、テールを切り落とします。応募者側にはこれに相当する仕組みがありません。30 〜 50 件の対象求人を目で 1 ページずつ読み、給与、勤務地、チーム規模、リモート方針すべてを同時にワーキングメモリで扱う羽目になります。8 件目あたりで疲弊し始め、最終的にはおすすめフィードの先頭をクリックして終わります。これは結局フィルタリング権限をプラットフォームに返す行為で、彼らの順序があなたの順序になります。',
      '104、LinkedIn、CakeResume の検索条件はハードフィルター（給与下限、地域、職種）しか表現できません。「成長 > 報酬 > 通勤」のような加重選好は表現できず、求人内容を自分の履歴書と照合することもできず、同じ検索の中で「広く探っている期間」と「オファーを手にして詰めている期間」で重みを切り替えることもできません。応募者は HR 側 ATS よりはるかに小さなバッチサイズで、自らマッチングエンジンの役割を担わざるを得ません。',
    ],
    solution: [
      'Job Ops は HR 側 ATS の応募者バージョン：同じ構造化採点ループを、候補者にではなく求人に向けたものです。macOS launchd が毎日 07:00 に Python パイプラインを起動し、104 から新着求人を取得。各求人は CV-aware evaluator に流れます。evaluator は私の markdown 履歴書を読み、YAML で定義した候補者アーキタイプと照合、採点を行い、結果を RECOMMEND / CAUTIOUS / SKIP の 3 段に分類。日報は HTML（朝のスマホ Gmail で直読）と Markdown（Obsidian アーカイブ用、後日の振り返り向け）の両方の形で生成され、07:30 までに Gmail で配信されます。',
      '8 つの採点ウェイト（報酬、リモート、技術スタック、成長、チーム、ブランド、勤務地、ライフ）は Python に直書きせず YAML ファイルに置いています。「広く探っている期間」と「オファーを詰めている期間」の切り替えは config/profile.yml の 1 行編集で済み、コード変更は不要です。このファイルは git で管理しているため、各コミットは私自身の優先順位が求職期間中にどう動いたかのタイムスタンプになります。求職が終わったとき、この 1 本の YAML ファイルの git history は、どの 1 日のレポートよりもこの活動全体の経緯を雄弁に語ります。',
      'パイプラインが自動化しきれない判断作業は、7 つのインタラクティブモードが Claude Code セッション内で引き継ぎます：cv-match（単一求人の CV 整合）、comp-research（市場給与調査）、legitimacy（企業と求人の合法性確認、コミュニティソースを参照）、level-strategy（IC vs マネジメント）、interview-prep、personalization、role-summary。パイプラインは決定論的な仕事（スクレイピング、重複排除、採点、配信）を担当し、インタラクティブモードは会話を必要とする判断を担当。lifecycle 状態（値変動、掲載終了、再掲載）は TSV scan-history に保存し、同じ求人が 3 回再掲載されても 1 行に diff 注釈付きで集約され、3 行のノイズに膨らみません。',
    ],
    techStack: [
    ...
    impact: [
      '毎日 30 〜 50 件の 104 新着求人が、RECOMMEND / CAUTIOUS / SKIP の 3 段に整理されたダイジェストとして 07:30 までに Gmail へ届く',
      '採点は私の markdown 履歴書と、YAML で定義した候補者アーキタイプを入力にする。求職スタンス（「広く探る期間」 vs 「詰める期間」）の切り替えは設定の編集で済み、コードを触る必要はない',
      '7 つのインタラクティブモードが、パイプラインに任せきれない判断を担当：企業の合法性確認（コミュニティソース横断）、IC vs マネジメント、報酬交渉、面接準備',
      'TSV scan-history は値変動と掲載終了を同一行の diff 注釈として表示。同じ求人が 3 日連続で投稿されてもダイジェストが汚れない',
      'macOS launchd でネイティブにスケジューリングし、Gmail App Password で OAuth なしの認証。house-ops と同じ自動化ファミリーを共有',
    ],
    learnings: [
      'エンジニアリングのピース（scraper、evaluator、ダイジェスト）は、どんな採点パイプラインにも共通します。守るに値する意思決定は「ルーブリックを誰に向けるか」です。HR は毎日 1 つのルーブリックを 100 人の候補者へ向けますが、このプロジェクトは毎日 1 つのルーブリックを 100 件の求人へ向けます。build を正当化するのは、長年気になっていた非対称性：応募者側で 30 〜 50 件の求人を仕分ける道具は自分の目と忍耐しかない、というものです。ルーブリックを反転させるのが一番安い修正でした。',
      '8 つの採点ウェイトを Python に直書きせず YAML に置いたことで、当初想定していなかったリターンが出ました。このファイルは git で追跡されているため、各コミットは自分自身の優先順位が求職中にどう動いたかのタイムスタンプとして残ります。求職が終わったとき、この 1 本の YAML の git log は、どの 1 日のレポートよりもこの活動全体を雄弁に語ります。振り返りは勝手に書き上がります。',
      'パイプラインとインタラクティブモードの分割は、house-ops で一度試して、job-ops でも改めて成立しました。決定論的な仕事（スクレイピング、重複排除、採点、メール配信）はパイプラインに残し、会話的な仕事（企業が本物かどうかの判断、IC vs マネジメントの議論、報酬交渉のロールプレイ）は Claude Code モードに移す。合法性の推論を Python モジュールに押し込めばパイプラインは肥大化し、毎日のスクレイピングを Claude セッションに押し込めば予算が膨らみ脆くなります。同じアーキテクチャの切り口が異なるドメインで成立し続けています。',
    ],
    links: [
