...
export const projects: Project[] = [
  {
    ...
    title: 'Path',
    description:
      'オフラインファーストの旅行プランニング PWA。Cache-first + background sync——海外でネットが落ちても、旅程・経路・コストにそのまま手が届きます。',
    ctaText: 'TRY IT',
...
export const projectDetails: ProjectDetail[] = [
  {
    ...
    solution: [
      'Path を**オフラインファーストの Progressive Web App** として作りました。アーキテクチャの問いは「native か web か」ではなく「ネットがなくても動く web app をどう作るか」でした。PWA は両方に答えました：iOS / Android のホーム画面に 1 つの URL からインストール可能、Service Worker で完全なオフライン能力、app store のゲートなし、native build なし、プラットフォーム税なし。',
    ...
    learnings: [
      'PWA は荷重を支えるアーキテクチャ判断でした。native へ行けば 2 つの codebase、2 回の app store 審査、それでも「ネットがなくなったらどうする」への良い答えはなし。PWA ルートは native が提供するもの（ホーム画面アイコン、オフライン対応、インストール可能性）をすべて Path に与えてくれました——プラットフォーム税なしで。さらに Service Worker + IndexedDB が今や web の first-class primitive なので、オフラインのストーリーは測定できるレベルで強くなりました。',
      'Cache-first + background sync は sequence diagram ではきれいに見えて、実装すると泥臭くなります。読み取りは素直——IndexedDB は cached データを即座に返します。難しいのは衝突：同じ旅行を 2 つのデバイスでオフライン編集、両方が後でオンラインに戻る。最終的には last-write-wins + timestamp ベースのマージ、加えてレコード単位の sync queue に落ち着きました。完璧ではないですが、実際のユーザー行動（単一ユーザー、ときどきマルチデバイス、本物の衝突は稀）に合っています。',
      'Google Maps API のコストはユーザー数ではなく経路検索数でスケールします。攻めの cache 戦略：経路は 24 時間 cache、ジオコーディングした場所は永続 cache、選択した transit polyline は旅程そのものに保存。利用が増えても API 支出は横ばいで、オフライン体験を損なうこともない——cache に当たるすべての呼び出しは、ネットなしでも動く呼び出しでもあります。',
    ],
    links: [
