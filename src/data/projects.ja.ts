...
export const projectDetails: ProjectDetail[] = [
  {
    ...
    solution: [
      ...
      '摩擦ゼロの行程編集：「14:00 浅草寺 2時間滞在 地下鉄で移動」のような自然言語を 1 行入力するだけで、Quick Add がローカルで（完全オフラインでも）時刻・滞在時間・場所・交通手段・メモを解析し、構造化された行程カードを生成します。開始時刻を持つカードは自動的に時系列で当日の行程に配置され、時刻未設定のカードは手動の並び順を保持。どのカードもその場で展開してインライン編集できます。',
      'セキュアなデータ分離：Supabase の Row Level Security (RLS) によって、マルチユーザー環境下のデータアクセスを安全に分離しています。',
    ],
    techStack: [
      { category: 'フロントエンドフレームワーク', items: 'React 18, TypeScript, Vite, TailwindCSS, shadcn/ui' },
  ...
  },
  {
    ...
    solution: [
      ...
      'Dev-discipline が実装フェーズを引き継ぐ：セッションが計画から実装へ移ると、軽量なエンジニアリング規律のレイヤーが引き継ぐ——TDD ファースト、スコープの完全性（範囲外の発見はフラグのみで黙って直さない）、シークレットとセキュリティの衛生、各マイルストーン後の二重のフレッシュ Context レビュー、そして仕上げのブランチチェックリスト。約 250 トークンの session-start ダイジェスト、1 つのオンデマンド skill、2 つの deterministic hook という軽さを意図的に保ち、システムの哲学どおり、決してあなたを強制停止させない。',
    ],
    techStack: [
      { category: 'Platform', items: 'Claude Code plugin（marketplace または install.sh でインストール、npm で公開）、Claude.ai Custom Skill' },
      ...
      { category: 'Guardrails & Continuity', items: '7 つの lifecycle hook（session-start での meta-skill と dev-discipline の注入・進捗ロード、離題の監視、計画フェーズの書き込みゲート、TDD gate、secret guard）、relative な非ブロッキング guardrail、scoped SessionStart ルーティング指令' },
      { category: 'Doc Processing', items: 'Playwright + pikepdf（ブックマーク付き PDF）、Pandoc（DOCX／PPTX）、pymupdf、Tesseract OCR フォールバック' },
      { category: 'Tooling', items: 'Node.js、Bash、Markdown ベースの lens 定義、2 系統の eval suite（trigger／behavioral）と deterministic Python scorer' },
