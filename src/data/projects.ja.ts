...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Plutus Trade',
    description:
      '台湾株向けの深掘り AI 分析ツール。Gemini がファンダメンタルズ、機関投資家フロー、テクニカルをクロスドメインで融合し、各戦略の長期勝率と実質 ROI を全面的にバックテストして追跡します。',
    ctaText: 'TRY IT',
  ...
  },
  {
    ...
    description:
      'LLM ベースの Multi-Agent System。3 名の専門家 Sub-agent を動的に編成し、22 種の定番プロダクトフレームワークに対応づけることで、曖昧な要件を高い実行可能性を備えた PRD 受け入れ仕様へ自動変換します。',
    ctaText: 'EXPLORE',
...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    subtitle: '「トレーディングデスク 1 卓ぶんの分析戦力を、AI 合成（Synthesis）1 回ぶんに収斂させる」。単一トレーダーのために設計された深掘り分析ツール。「ファンダメンタルズ + 機関投資家フロー + テクニカル」の従来は数時間かかる研究サイクルを、論理的な推論を伴う 1 回の意思決定提案に圧縮します：全次元の合成（月次売上、ファンダメンタルズ、機関投資家フロー、テクニカル構造を Gemini AI がクロスドメインで診断）、監査可能な意思決定（各 AI 推奨は「Instrumented」処理され、長期の勝率と ROI を追跡）、量的下層アーキテクチャ（量的モメンタムモデルで候補プールを絞り込み、AI の意味解釈が確かなデータに支えられるよう保証）。',
  ...
  },
  {
    ...
    solution: [
      'Multi-Agent スペック生産パイプライン：Product Playbook は厳密なスペック生産パイプラインとして稼働する。v1.2 で Multi-Agent 専門家分業アーキテクチャへ進化した。',
    ...
    learnings: [
      ...
      'チャネル戦略がユーザーリテンションを決める。Skill ベースの配信戦略を選んだことで非常に高い定着率を獲得した。優れたツールは利用者がすでに作業している場所へ「会いに行く」設計を採用する。',
