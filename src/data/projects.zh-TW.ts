...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Plutus Trade',
    description:
      '台股深度 AI 分析工具。由 Gemini 跨領域揉合基本、籌碼與技術面，每日以真實大盤與供應鏈數據生成持股策略，並追蹤每筆建議的長期勝率與實質 ROI。',
    ctaText: 'TRY IT',
...
export const projectDetails: ProjectDetail[] = [
  {
    ...
    solution: [
      'Path 從底層架構重新定義「沒網路也能跑的 Web App」。',
      ...
      '零摩擦的行程編輯：輸入一句「14:00 淺草寺 停留2小時 搭地鐵前往」，Quick Add 就在本地端（完全離線可用）解析出時間、停留時長、地點、交通方式與備註，直接生成結構化行程卡片。帶開始時間的卡片會自動依時間順序排入當日行程，未定時的卡片維持手動排序；每張卡片也都能原地展開直接編輯。',
      '安全隔離：利用 Supabase 的 Row Level Security (RLS)，確保多使用者環境下的資料存取安全。',
    ],
    techStack: [
      { category: '前端框架', items: 'React 18, TypeScript, Vite, TailwindCSS, shadcn/ui' },
  ...
  },
  {
    ...
    subtitle: '「將一整張交易桌的分析戰力，收斂進一次 AI 合成 (Synthesis)。」這是一套專為單一交易者打造的深度分析工具。它將原本耗時數小時的「財報 + 籌碼 + 技術面」研究循環，壓縮成一次具備邏輯推論的決策建議：全維度合成（整合月營收、基本面、法人籌碼與技術結構，由 Gemini AI 進行跨領域診斷）、可稽核的決策（每一筆 AI 推薦都經過「儀器化（Instrumented）」處理，長期追蹤勝率與 ROI）、量化底層架構（以量化動能模型篩選候選池，確保 AI 的語意推論建立在紮實的數據基礎上），以及有數據根據的持股策略（每日先以真實大盤情境與個股基本面富集每一檔持股，再交給 Gemini 撰寫當日的進出場策略）。',
    ...
    solution: [
      ...
      '有數據根據的每日持股策略：每天 16:30 先組好一份真實數據情境（大盤 ^TWII 趨勢、美股三大指數與費半、宏觀新聞），再加上每一檔持股的基本面富集（近 60 日趨勢、三大法人淨買賣超、EPS、月營收 YoY/MoM、供應鏈新聞），然後把這些證據交給 Gemini。每一檔回傳時都帶有觀察重點、賣出目標價、加碼或減碼的理由，以及供應鏈與地緣風險，並於每晚以 Email 寄給交易者。',
      '常駐自動化：以排程 cron 執行盤後日報（勝率追蹤、預測驗證、法人動向、明日策略與 AI 自我檢討）、16 種 Web Push 智慧通知（含 12 小時冷卻），以及 Threads 達人觀點監測，把市場意見領袖的新貼文蒸餾成每日摘要。',
    ],
    techStack: [
      { category: 'Frontend', items: 'Flutter 3.41+（Riverpod、go_router；跨平台支援，部署於 Vercel）' },
      ...
      { category: 'Data Enrichment', items: '每日大盤情境與個股基本面富集服務（^TWII、美股、費半、三大法人、EPS、營收；Redis 共用快取）' },
      ...
      { category: 'Notifications & Email', items: 'Web Push（VAPID、pywebpush）、Resend（交易型 Email）' },
      { category: 'Deployment', items: 'Fly.io（後端 nrt region）+ Vercel（前端）' },
    ],
    impact: [
      '全功能覆蓋：單一界面整合市場數據、AI 診斷、導引選股、預測追蹤、每日持股策略、盤後日報與智慧通知等 8 個模組',
      '數據韌性保證：透過三層資料源 Fallback 與 7 天 Stale-cache 安全網，確保在上游服務退化時，工具依然具備分析能力',
      '可審核的信任：每一筆 AI 呼叫皆可溯源與結算，解決了 AI 在金融決策中常見的「黑盒子」問題',
      '數據接地的 AI 產出：大盤情境與個股富集層會在 Gemini 撰寫持股策略前，先餵入真實的三大法人、EPS 與月營收，讓每一個目標價與建議都有實際數據支撐',
    ...
    learnings: [
      ...
      '「接地」才是信任的來源：可靠度最大的提升，來自把真實數據餵給模型。我打造了大盤情境與個股基本面的富集層，讓 Gemini 以實際的三大法人、EPS 與月營收進行推論，這正是讓每日持股策略可靠到足以據以行動的關鍵。',
      '產品設計的邊界感：在金融產品中，「分析」與「建議」的界線必須在介面層強制切割。產品的工作是將模型回應框架在附帶免責聲明的專業分析中。',
  ...
  },
  {
    ...
    solution: [
      ...
      'Dev-discipline 接手實作階段：當對話從規劃走進實作，一層輕量的工程紀律接手——TDD 先行、範圍完整性（範圍外的發現只標記、絕不偷改）、金鑰與資安衛生、每個里程碑後的雙重全新 Context 審查，以及收尾分支檢查清單。它刻意保持輕量（約 250 token 的 session-start 摘要、一個隨需 skill、兩個 deterministic hook），並延續整套系統的哲學：永遠不硬性擋下你。',
    ],
    techStack: [
      { category: 'Platform', items: 'Claude Code plugin（marketplace 或 install.sh 安裝，發佈於 npm）、Claude.ai Custom Skill' },
      { category: 'AI Engine', items: 'Claude（meta-skill lens 編排）、Claude Vision（語意文件解析）' },
      ...
      { category: 'Guardrails & Continuity', items: '7 個 lifecycle hook（session-start 注入 meta-skill 與 dev-discipline、載入進度、偵測離題、規劃期文件把關、TDD gate、secret guard），relative 非阻斷 guardrail，scoped SessionStart 路由指令' },
      { category: 'Doc Processing', items: 'Playwright + pikepdf（帶書籤的 PDF）、Pandoc（DOCX／PPTX）、pymupdf、Tesseract OCR 備援' },
      { category: 'Tooling', items: 'Node.js、Bash、Markdown-based lens 定義、雙 eval suite（trigger／behavioral）搭配 deterministic Python scorer' },
      { category: 'Localization', items: 'Runtime 語言偵測，Claude 以使用者的語言回覆；lens 內容以英文撰寫' },
    ],
    impact: [
      '開發效率極大化：將原本需要數天的規格撰寫過程大幅縮短至數分鐘，且品質超越基準測試',
      '+69% 品質提升，實測數據：在早期架構的 Iteration 4 評測中，相較於未掛載技術的同一個模型，產品思維與邏輯嚴整度提升 69%，這正是把系統做大的理由',
      '專家分工是承重結構，並延續到 2.0：啟用早期的 Sub-agent 層後，品質完成率從 59.1% 提升到 100% 且 Token 成本持平；單獨移除 pre-mortem 會讓同一組評測從 100% 掉到 22.2%。正是這份證據，讓 strategy-critic 與 pre-mortem-runner 在 lens 架構裡繼續擔任專屬專家',
      '2.0 更精簡也更清晰：16 個可組合 lens、每次輸出的 provenance 與非阻斷 guardrail，讓工具在探索到交接的整段弧線上自行選對框架',
      '工作流無縫融合：以 Claude Code 與 Claude.ai skill 發佈，直接在編輯器或終端機呼叫，無需切換平台',
      '開源社群貢獻：以 MIT 協議開源，成為 PM 與工程師跨團隊協作的生產力工具',
    ],
    learnings: [
      '2.0 的改寫是一場對「組合」的下注。1.x 的設計要我先選一個 mode，再走一條固定的流水線；但觀察真實使用後發現，多數產品問題一次只需要一兩個 lens。把每個框架重建成可組合的 lens，再配上 outcome-first 的路由，讓工具只帶上問題需要的思考，而 provenance 那一行讓這個選擇保持誠實、可檢視。',
      '產品價值住在「框架」那一層：作為 AI PM，我體會到 LLM 的編排本質上是個產品設計問題。生成只是表象，哪個 lens、在什麼時機執行才是核心：唯有先定義 Persona 再執行 JTBD 分析，AI 才能精準識別出真正的用戶痛點。',
      '授權專家（Empowered Specialist）是一個核心的工程決策：過去我以為 Sub-agent 只是把長 Prompt 拆短的技術優化，但實作後才發現，「拒絕路徑」與「唯讀工具集」才是設計核心。允許專家在越權時說「這不是我的工作」並交還決策權，正對應 Marty Cagan 所說的「自主團隊」概念，而清楚自己邊界的專家反而讓系統更穩定。評測也證明了這點：少了 pre-mortem，風險步驟會掉到 22.2%，因此 strategy-critic 與 pre-mortem-runner 都在 2.0 改寫中以專屬專家的身分留了下來。',
      '架構的演化在於「誠實面對差異」：設計初期我曾想讓所有 Sub-agent 共用一套工整的 YAML Schema。但實作發現，策略批判需要的是「盲點評分」，而風險推演需要的是「帶指標的場景模擬」，強行統一只會稀釋專業度，最終我採取「共用外殼（Envelope）＋各自內核（Body）」的做法。一致性的價值在於服務品質目標；當底層問題本質不同時，強行標準化反而是種傷害。',
      '核心洞察：任何人都能呼叫 API，但知道如何「問對問題」、「導引思考」並「讓專家在對的時機說話」才是系統真正的壁壘。lens 是這套系統的靈魂，outcome-first 的路由是它的骨架，而 AI 則是負責快速交付的自動化機制。',
    ],
    links: [
  ...
  },
  {
    ...
    problem: [
      '對於在台灣尋找租屋或買房物件的人來說，最大的痛苦在於「看不完」。',
    ...
    learnings: [
      '「在自動化的世界裡，交付媒介就是產品本身。」為什麼選 Email 當交付媒介？這源於對行為科學的洞察：在早晨的高頻決策時段，「推播（Push）」的資訊到達率遠高於「拉取（Pull）」。數據應主動出現在使用者的注意力所在之處。',
