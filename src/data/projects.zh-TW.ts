...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Job Ops',
    description:
      '每天早上 7 點，一條 Python pipeline 把當天 104 新上的職缺抓回來，對照我的履歷與個人優先順序（薪資、遠端、成長、團隊、地點）逐筆打分，再把推薦 / 觀察 / 跳過三段排好寄到信箱。需要更深入研究某份職缺時（這間公司是不是真的、要選 IC 還是當主管、面試怎麼準備），7 個 Claude Code 互動模式接手用對話處理。',
    ctaText: 'EXPLORE',
...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    subtitle: '一條個人 Python pipeline，把 HR 端 ATS 對候選人做的事（結構化評分、每日排序）反過來指向職缺：macOS launchd 每天 07:00 觸發，從 104 抓當天新上的職缺，用我的履歷與個人優先順序逐筆打分，分為推薦 / 觀察 / 跳過三段，07:30 前由 Gmail 送進手機。需要對話深度的判斷由 7 個 Claude Code 互動模式在 session 內接手：公司合法性查核、職等策略、薪資調查、面試準備。',
    metaTitle: 'Job Ops — 候選人視角的個人版 ATS | Charles Chen 個人作品',
    metaDescription: '個人化的 Python pipeline 每天早上 7 點抓 104 新職缺，對照我的履歷逐筆評分，把推薦 / 觀察 / 跳過三段寄到信箱。搭配 7 個 Claude Code 互動模式處理公司合法性查核、職等策略、薪資調查、面試準備。AI 產品經理 Charles Chen 的個人求職 OS。',
    problem: [
      '求職的 HR 那一端有 ATS，每天用一套結構化規則把 100+ 個候選人打分、排序、淘汰。候選人這一端沒有對應工具。30 到 50 個目標職缺只能靠人眼一頁頁讀，薪資、地點、團隊規模、遠端政策全塞進腦袋同時比較。讀到第 8 個職缺就開始懶了，最後乾脆點推薦列表第一個進去看，等於把篩選權交還平台：他們的排序變成你的排序。',
      '104、LinkedIn、CakeResume 的搜尋條件只能設硬性條件（薪資下限、地區、職類）。沒辦法表達「我重視成長性 > 薪資 > 通勤」這種加權偏好；沒辦法用我的履歷比對職缺敘述；也沒辦法依求職階段（剛開始海撒履歷時 vs 手上已經有 offer 在收網時）動態調整評估權重。候選人被迫自己當匹配引擎，處理量遠小於 HR 端 ATS 跑得動的批次。',
    ],
    solution: [
      'Job Ops 是候選人視角的個人版 ATS：把 HR 端那一套結構化評分流程反過來，指向職缺。macOS launchd 每天 07:00 觸發 Python pipeline，從 104 拉當天新上的職缺，每筆都丟進 CV-aware evaluator：讀我的 markdown 履歷、對照定義在 YAML 的候選人原型、逐項評分、分成推薦 / 觀察 / 跳過三段。報告以 HTML（早上手機 Gmail 直讀）和 Markdown（同步進 Obsidian 留存供月底回顧）兩種格式，在 07:30 前送進信箱。',
      '8 個評分維度（薪資、遠端、技術棧、成長、團隊、品牌、地點、生活）的權重放在 YAML 不寫死在 Python。從「海撒履歷期」切到「收網期」只要改 config/profile.yml 一行就好，evaluator 程式碼不必動。這份 YAML 進 git 後，每次 commit 都是自己優先順序變化的時間戳；求職結束後拉這支 YAML 的 git log 出來看，會比任何一份單日報告更能還原這段求職的整體軌跡。',
      '7 個互動模式接手 pipeline 不適合自動化的判斷工作，全部在 Claude Code session 裡跑：cv-match（單一職缺 CV 對位）、comp-research（市場薪資調查）、legitimacy（公司與職缺合法性查核，背後串論壇來源）、level-strategy（IC 路線 vs 管理職）、interview-prep、personalization、role-summary。Pipeline 處理 deterministic 工作（爬、去重、評分、寄信），互動模式處理需要對話深度的判斷。lifecycle 狀態（價變、下架、重貼）存在 TSV scan-history，同一職缺貼三次只會顯示一列加上 diff 註記，不會累積成 3 列噪音。',
    ],
    techStack: [
    ...
    impact: [
      '每天 30–50 個 104 新職缺，收斂成推薦 / 觀察 / 跳過三段日報，07:30 前送進手機',
      '評分流程吃我的 markdown 履歷加上定義在 YAML 的候選人原型；從「海撒履歷期」切到「收網期」是改 config，不是改 code',
      '7 個互動模式補完判斷層：公司合法性查核（背後串論壇來源）、IC vs 管理職、薪資談判、面試準備',
      'TSV scan-history 把價變、下架、重貼以 diff 註記呈現在同一列；同一職缺貼三次不會在日報上佔三列',
      '排程走 macOS launchd 原生，認證走 Gmail App Password（沒有 OAuth 折騰），跟 house-ops 共用同一套自動化 family',
    ],
    learnings: [
      '工程組件（scraper、evaluator、digest）跟任何評分 pipeline 都一樣，真正值得守住的決策是「這把尺指向誰」。HR 端每天把同一把尺指向 100 個候選人；這個專案每天把同一把尺指向 100 個職缺。Build 的價值主張，是補上那個困擾我多年的不對稱：候選人這一邊，分類 30-50 個職缺的工具只有自己的眼睛跟耐性。把尺反過來，是最便宜的修法。',
      '把 8 個評分權重外接到 YAML（而非寫死在 Python）帶來的回報超出我原本的預期。因為 YAML 進了 git，每次 commit 都是「我自己優先順序在這次求職中怎麼移動」的時間戳。求職結束後，這支 YAML 的 git log 比任何一份單日報告都更能還原整段歷程，retrospective 自動就寫好了。',
      'Pipeline + 互動模式的雙層切法，在 house-ops 上試過一次，在 job-ops 上再次成立。Deterministic 工作（爬、去重、評分、寄信）放 pipeline；對話型工作（判斷公司是不是真的、IC 還是當主管、薪資談判的話術）放 Claude Code 互動模式。試圖把合法性推理塞進 Python 模組會讓 pipeline 膨脹到不像話；試圖把每天的 scraping 塞進 Claude session 會燒錢且容易壞。同樣的切法在不同領域反覆成立。',
    ],
    links: [
