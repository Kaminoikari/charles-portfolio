...
export const projectDetails: ProjectDetail[] = [
  ...
  },
  {
    ...
    subtitle: '「將一整張交易桌的分析戰力，收斂進一次 AI 合成 (Synthesis)。」這是一套專為單一交易者打造的深度分析工具。它將原本耗時數小時的「財報 + 籌碼 + 技術面」研究循環，壓縮成一次具備邏輯推論的決策建議：全維度合成（整合月營收、基本面、法人籌碼與技術結構，由 Gemini AI 進行跨領域診斷）、可稽核的決策（每一筆 AI 推薦都經過「儀器化（Instrumented）」處理，長期追蹤勝率與 ROI）、量化底層架構（以量化動能模型篩選候選池，確保 AI 的語意推論建立在紮實的數據基礎上）。',
