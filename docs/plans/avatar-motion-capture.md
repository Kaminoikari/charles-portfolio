...
畫框：launcher/docked 是 y 0.618–1.722、兩側都是 ±0.674；fullscreen column 是
...
- **不准沉出畫面**：每一格的髖部要高過該畫框的下緣（waist-up 0.618、column 0.430）。
  這是管「動作做了什麼」的那一道。`squat` 最低 0.660，兩個畫框都過。
...
- 逐支用 `__mikaHandle.playMotion()` 驅動，十支的髖部高度都會動，
  ...
  所以手臂確實由 clip 驅動
- **未驗**：`stretch` 手過頂那一格的實際畫面。截圖與 evaluate 是兩次往返，兩次都落在
