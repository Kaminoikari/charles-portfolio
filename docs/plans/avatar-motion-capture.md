...
| `AVATAR_LAUNCHER_BODY_FRACTION` | 0.415 | 0.3775 | ×342/376（2026-08-22 改名 `AVATAR_WAISTUP_BODY_FRACTION`，並改由實測左右緣 0.30／0.69 相減得出 0.39） |
| `AVATAR_LAUNCHER_HIT_INSET_PCT` | 24 | 26 | 點擊區絕對寬度維持（實測 180px vs 178px） |
| `AVATAR_BUBBLE_RIGHT_PX` | 256 | 273 | 對話框仍在身體左緣外 14px（2026-08-22 更正：14px 出自「她置中」的錯誤模型，用實測左緣算是 **9.8px**） |
...
（使用者 2026-08-19 已接受手勢被截）。
（**2026-08-22 修訂：這段的最後一句當時就是假的。** 684／752 ＝ 0.91，而讀畫布像素
實測她的輪廓只佔畫布寬的 0.39（左右緣 0.305／0.686）。那個分母護住的是她左右各 0.21
的空氣，代價是每個會觸發縮放的視窗都把她無謂縮小。分母已改成實測的身體框，常數本身
刪除。）對應的測試也從「畫布不出界」改成「身體不出界」，
並且反過來釘住「在最窄的可達視窗時畫布確實出界」，免得斷言被悄悄放寬（寫這段時
那是 880px；2026-08-22 閘門改成 `besidePanelFits`，該斷言改成掃出當下最窄的可達寬度）。
...
（2026-09-07：上表兩個值各動一格，`waistUp` **-0.07**、`column` **+0.14**。理由不是重新
取捨，是 VRoid 身體第一次被自己模擬起來之後髮頂比上表用的瀏覽器值高了 11mm，
`clearance.panFor` 用同一套算術重推的結果。詳見本節末的 2026-09-07 補述。）
...
有生產者了，見本節稍後的「Phase 5」補述；`dance` 這個 1.7276 當時仍是上界，因為瀏覽器
掃到的值比從 Milfy 身體轉移過來的 1.7177 高（那是 +0.13 那時的轉移值；+0.14 之後轉移值
是 1.7165）。2026-09-07 起上界是 VRoid 身體自己模擬出
來的 1.7389，那組瀏覽器值退成第二意見。）
...
（2026-09-07：變成七支。`playFingers` 與 `scratchHead` 在 column 各拿到 +0.02，理由不是
身體會移動，是頭髮：VRoid 身體自己的模擬把這兩支的髮頂抬到上緣之上，而它們原本靠的
是 `crownTop` waiver。上面那句「只有會移動的那一支需要鏡頭配合」因此不再是全部理由，
第二個理由是「站著不動但頭髮出框」。）
...
所以「記錄看過最糟的」這條規則沒有被自動化沖掉。（2026-09-07 起 `crownBound` 取的是
三者最大：轉移值、這組掃描值，以及該 family 每一具自己模擬出來的髮頂。）
...
（2026-09-07：`dance` 的 column pan 從 +0.13 動到 +0.14、waistUp 從 −0.08 到 −0.07，
因為 VRoid 身體自己的模擬把這支的髮頂抬高了 11.3mm（column）／11.1mm（waist-up）。
上面這段的數字是 +0.13 那時量的，窗口與方向都不變，只是上緣現在是 1.7420。）
...
2026-09-07 起**只剩三支**（收據 `scripts/avatar/evidence/crown-0907.md`）。VRoid 身體
第一次被自己模擬起來（`springsim.deriveManifest` 解除了「沒有 manifest 就模擬不了」），
結果 `scratchHead` 與 `playFingers` 的髮頂又比上面那組掃描值高 7–9mm。這兩支沒有加寬
waiver，改吃 `clearance.panFor` 推出來的 `column: +0.02`：waiver 是擁有者看過並接受的
讓步，pan 是量出來的推導，而沒有人在新高度上接受過那兩支。相機抬高之後它們根本不出
上緣，於是「用不到的 waiver 一樣是失敗」這條規則自己把它們刪掉了。`spin`、`squat`、
`idleLoop` 三支照舊。
