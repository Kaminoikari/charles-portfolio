...
起點，不是今天**。今天（`107f357`）`build.py` 是 1613 行
（`git show HEAD:scripts/avatar/build.py | wc -l`）、模組層常數 10 個，簽章從四個參
數變成七個；差異在文末各步驟的產出裡。
...
- 每一步結束後，用同一組輸入跑 `make.py`，產出的 **manifest 逐欄相同**，且所有
  ...
  （**實際沒有用 `make.py` 問這個問題**，理由與改用的做法見文末「驗收的產出」。）
...
### 二十二處 inline 的 VRoid 名字，是步驟 1 看不到的那一種

在步驟 4 之前的 `8de2d67` 上，`F00_000` 這串文字出現 **24 次、分佈在 20 行**
（`git show 8de2d67:scripts/avatar/build.py | rg -o 'F00_000' | wc -l`，行數用
`rg -c`）。其中 **2 次在 2 行**是模組層常數 `HEAD_HAIR` 與 `OUTLINE_KEEP`，這兩個
步驟 1 有找到，就列在上面的底模軸裡。**剩下 22 次、18 行，才是步驟 1 看不到的那一
種**：寫在函式內的字面量。

它們拼出 **12 個相異的名字**（六張髮貼圖、眉、臉 atlas、身體 atlas、虹膜、兩個膚材
質），這個數字要對展開後的字串數才算得出來，因為六張髮貼圖由同一個 f-string 產生，
字面上只看得到一個前綴；可重跑的算法是數 `bodies/mika_base.py` 宣告的 `F00_000` 字
串（見該檔開頭）。

先前寫的「19 處」對不上任何一種口徑，而第一次重數寫成的「24 處 inline」把兩個真的
是常數的算了進去。**步驟 1 的盤點漏掉的是那 22 處**，因為一個寫在函式內的名字不是
常數。這是比計畫預期更大的耦合面，也是「列常數」這
...
重現誤差以 `build()` 自己在 `out/proportioned.vrm` 上算出的地標為準。地標要帶足位
數才重現得出微米級的差，四捨五入到小數第六位（也就是 0.001mm）就不夠：waist
0.959999986、shoulder 1.21511054、hip 0.843383433、knee 0.500952433。據此
**最差 0.0035mm，
...
## 驗收的產出：同一組輸入，兩棵樹（2026-09-11）

**`make.py` 不能拿來問這個問題。** 它最後一步會複製覆蓋
`public/avatar/mika-milfy-12.vrm`，那是網站以 cache-immutable 帶版本名對外服務的
檔案；而這條管線本身有已知的位元組非決定性（`head.py` 的兩顆髮髻）。用它驗收等於
拿一個會漂移的量尺去覆蓋一個線上產出物。

改成兩支腳本，把「跑整條管線」與「跑 `build()`」拆開：

- `evidence/build-e2e-0911-prep.py` 從 `baseline.vrm` 跑 make.py 的步驟 0b–4
  （VRM 1.0 入口轉換、partition、strip、皮膚重繪、比例），輸出到 scratch 目錄。
- `evidence/build-e2e-0911-run.py` 拿**一棵樹**的 `build()` 跑那組輸入並印出產出。
  把它指向重構前的 `git archive`，再指向工作樹，兩份答案可直接比對。

比對 `135d33b`（角色合約搬家前的最後一個 commit）與 `30488d3` 加上遮蔽修正：

| 量的東西 | before | after |
|---|---|---|
| manifest 逐欄走完 26 個部件 | identical | identical |
| manifest sha256 | `aa49410c4a4a2612…` | same |
| VRM 位元組 | 12034560 | same |
| `build()` 新增的部件數 | 32 | same |
| landmarks | identical | identical |

完整收據在 `scripts/avatar/evidence/build-e2e-0911.md`。

### 它抓到 487 條單元測試抓不到的東西

`edge` 在整個 `build()` 裡持有 `torso_edges(lm)`，而鈕扣迴圈把它重新綁成一個頂點。
步驟 4 讓胸前荷葉邊去讀一個 torso edge，位置在那個迴圈下方三百行，於是它拿字串去索引
numpy 陣列而爆掉。所有單元測試照樣全綠，因為沒有一條會端到端跑 `build()`：合約測試
量的是接縫，而接縫是對的。

同一種遮蔽當天發生兩次（另一次是區域變數 `hair_materials` 遮蔽新的模組層函式）。
`build_test` 因此多了 `DerivedOnce`：對 `build()` 推導一次、遠處才讀的那些名字，斷
言每個恰好被綁定一次，而且斷言它們還在，因為一個消失的名字會免費通過前半條。數量
刻意不寫在這裡，它只寫在 `WATCHED` 那一行。
...
這三條加起來動了 `build.py` **36 處**字面量（31 個 `Milfy_*`、1 個光禿禿的
`'Mellow_'`、4 個 `'Leg_belt'`；`git show 8b188d2 -- scripts/avatar/build.py | rg '^-'`
再數這三種 pattern）。31 是第一條自己的數，先前被當成三條的總和。從 `baseline.vrm` 重跑 make.py 步驟 0b–4 再跑兩
...
- 步驟 1 的收據改成分類它**當時**分類的那個 blob，另外保留一道對今天 `build.py` 的
  ...
  個 mutation，因為「新增一個沒有軸的常數」會同時觸發兩道，遮蔽掉第二道。收據在
  `evidence/build-axes-0911-mutations.md`，原始輸出在同名 `.log`。
