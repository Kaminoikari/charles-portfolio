...
### 1. `skin.py` 的兩個「對這具身體剛好夠用」的常數

量的方式：把每具身體的 body skin atlas 跑一次 `strip()`，殘留定義為「`is_skin` 判定
不是皮膚、且落在該 atlas 被 mesh 實際取樣到的 UV 範圍內」的 texel 比例。UV 範圍由
Body SKIN 材質的 UV 三角形逐一光柵化得到。

**1a. pull-push 的金字塔深度（已修）。** 原本寫死 `levels=9`，在 2048 見方的 atlas 上
只降到 4×4。AvatarSample_A 與 AvatarSample_C 在那一層各有 2/16 個格子完全沒有有效
像素，`colour / max(weight, 1e-6)` 於是輸出 `(0, 0, 0)`，上採樣再把這個零一路混進底下
每一層。AvatarSample_A 洞內的填色中位數因此是 `[175 148 130]`、最暗處 `[49 41 36]`，
而存活皮膚的平均是 `[243 215 190]`。

改成一路降到 1 像素之後：

| 身體 | 修正前殘留 | 修正後 |
|---|---|---|
| AvatarSample_A | 12.46% | **0.04%** |
| AvatarSample_C | 14.05% | **0.00%** |
| 其餘 14 具 | 0.00%–0.44% | 同左，逐項不變 |

Mika 自己的遮罩逐 texel 相同（重繪比例兩邊都是 36.1008%），只有填色改變，最大單通道
差 7/255、平均 1.59；出貨檔 `mika-milfy-12` 的 atlas 最大差 1，影響 0.058% 的 texel。

**1b. `is_skin` 的絕對門檻（未修）。** `r > 105` 把 AvatarSample_A 的深棕上衣收進皮膚
色域：56,742 px、中位色 `[120 92 80]`，修完金字塔之後它仍以兩條深棕直條留在圖上。

這個缺陷**殘留指標看不見**，因為指標問的是「`is_skin` 認不認為它是皮膚」，而它正是被
`is_skin` 誤收的那一塊。要另立一條不依賴該述詞的判準。

已經試過並否決的做法：把 `r > 105` 換成「亮度不得低於參考亮度的 k 倍」，k 掃
0.50／0.60／0.70。AvatarSample_A 只從 12.46% 動到 12.25%，而 Darkness_Shibu 從 0.44%
壞成 24.74%。原因是當時金字塔還是壞的，遮罩一變大洞就變大，黑格子就跟著出現。金字塔
修好之後這條路要重測。

參考色的來源已經驗證可用：依 humanoid map 取手部與手指骨權重 ≥ 0.9 的頂點，沿 UV 取樣
atlas 取中位數。**16 具身體每一具都拿到 1428 個頂點**，各自的中位色是
AvatarSample_A `[254 231 205]`、AvatarSample_B `[206 157 135]`、mika-pink
`[243 177 163]`。
