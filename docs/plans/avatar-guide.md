...
**這是真的被裁，而且我前兩次都量錯地方。** 寬度檢查一直只看 `ARM_GESTURE_PEAKS`
的**終點**。手臂從垂下抬到折起的路徑上，前臂會先在額狀面裡掃過水平，那一瞬間
指尖到中線 0.622，畫布半寬只有 0.484，超出 29%。兩個端點反而都很窄（rest 0.233、
姿勢本身 0.31），所以峰值檢查看不到。

- **檢查改成走完整段路徑**：`armAt(pose, env)` 是引擎與檢查共用的姿態來源，
  `poseReach()` 沿 env 取 96 個樣本取最大。引擎不可能走到檢查沒看過的形狀。
- **修法是轉場中把肩膀往前轉**：`ARM_TRANSIT_YAW = 1.3`，以 `sin(env·π)` 進出，
  所以兩端都是 0，**她停住的姿勢完全沒變**。只套用在「前臂往上折」的姿勢
  （`pose.fore < ARM_REST_FORE_Z`），往下的（插腰）本來就不寬。
  路徑最寬點 0.622 → 0.392，對 0.484 有 1.24 倍餘裕。
- **繞 x 軸前擺救不了**：x 旋轉會保留其下所有骨頭的 x 分量（實測：1.25rad 前擺
  讓靜止手腕從 0.212 只移到 0.207）。必須是繞 y 的肩膀轉向才會縮短側向投影。
  時序調整（前臂先折、上臂後抬）最多只能壓到 0.509，仍然超。
- **像素驗證**（讀 canvas 的 alpha，不受背後星空干擾，245×280 launcher）：
  yaw = 0 時 doublePeace 在 t=0.22s 有 58 個像素貼在畫布邊緣、handsBehindHead 56 個；
  yaw = 1.3 後整段抬手的最窄邊距是 24px，邊緣像素 0。
- 量測工具的注意事項：`getBoundingClientRect` 會把入場的 scale transform 算進去
  （量到 176×202 而不是 245×280），而畫面亮度門檻會把 hero 星空誤判成她。
  用 `gl.readPixels` 的 alpha 通道，並等 `matz === 2`。
