...
存在多個世代**：這三支跳到 `-zh3`，其餘仍是 `-zh2`（下一節的 `bye-1` 又加入 `-zh3`，
而 `intro-1` 再往前走到 `-zh4`）。沒有整組改名，因為 stage 1 是
隨機取樣，重抽只會弄丟使用者已經認可的 take。接線在 `avatarVoice.ts` 的 `ZH_REGEN`：
`localised()` 從「每個 locale 一個後綴」改成「預設後綴加上逐 clip 覆寫」，
`avatarVoice.test.ts` 用一份寫死的清單釘住是哪幾支（清單刻意不從實作 import，改映射
必須動兩個地方；那份清單後來隨著 `bye-1` 與 `intro-1` 一起長大）。`gen_visemes_align.locale_of()` 的 regex 本來就允許世代編號，`-zh3` 直接通過。
...
播放器會跳過」，證據是當時的 `mika-intro-1-zh2`。這一輪在取代它的那批上重量，ffprobe 讀
m4a 與讀來源 wav 的差距是 0 到 1ms，那個位移量不到了。原因沒查（不在這一輪範圍內），
註解已改成同時記載兩次量測。嘴型軌照舊對 wav 計時：那個做法在兩種情況下都是對的。


## 音高是被轉換抬上去的，不是台詞的問題（2026-08-21）

上一節收尾之後，使用者又點了 `intro-1` 與 `bye-1`。這一輪的價值不在改了什麼，在於
**使用者自己把故障分到了正確的階段**，而那個分法直接決定了修法。

### `intro-1`：先是音高，再是名字

使用者聽完 stage 1 對 stage 2 的並列之後說「原始語音沒問題，但是換完音色之後音頻變得
太高了」。這一句把問題釘死在 stage 2，而 stage 2 確實有一個會做這件事的機制：
`auto_f0_adjust` 把來源的 F0 曲線**整體移調到參考音的中位**。參考音是つむぎ，中位
351Hz，所以來源偏低的 clip 會被抬過頭。

實測（中位音高）：

| clip | stage 1 | stage 2 | 差 |
|---|---|---|---|
| `intro-1` | 229Hz | **359Hz** | +7.7 半音 |
| `suggest-2`（使用者接受） | 213Hz | 327Hz | +7.4 半音 |
| `bye-1` | 325Hz | 343Hz | +0.9 半音 |

`intro-1` 的 359Hz 比參考音本身還高。`vc_to_tsumugi.py` 因此長出 `--semi-tone-shift`，
在 `auto_f0_adjust` 之後再整體移調；使用者從 −1.5／−3／−4.5 三階裡選了 −1.5（331Hz，
正好落在他接受過的 327Hz 旁邊）。

**這個修正是資料，不是指令列上的旗標。** 第一版只有旗標，reviewer 指出那會靜默失效：
整批重跑而沒人記得帶 `--semi-tone-shift -1.5`，`intro-1` 就回到 359Hz，而**沒有任何測試
會紅**——嘴型軌、catalogue、檔名三者仍然彼此吻合，錯的只有聽起來的音高。修法是把它移進
`voice_lines.PITCH_SHIFT`（與台詞同一個擁有者），`vc_to_tsumugi.pitch_for()` 逐 clip
查表，旗標降級成覆寫用。不在表裡的 clip 拿 0，那是對的預設。

**表的 key 本身踩過兩次坑，值得照抄結論**：`mika-intro-1-zh` ——留 locale、脫掉世代編號。
第一版寫完整的 `mika-intro-1-zh4`，reviewer 指出那只是把同一個失效往後挪一格：`intro-1`
下次重錄成 `-zh5` 就查不到，`.get()` 靜默回 0.0，音高回到 359Hz。改成完全不帶後綴的
`mika-intro-1` 又壞另一邊：那個名字同時是英文與日文版的前綴，`mika-intro-1-en2` 會繼承
一個照中文來源量出來的修正（這個迴歸是自寫的 case 表抓到的，不是靠讀）。另外，
`voice_lines.py` 尾端有一行 module-level assert 擋打錯字：表裡的 key 對不到任何 clip 就在
import 當下炸，因為 `scripts/` 沒有測試 harness，這是唯一會在打錯時出聲的地方。

音高定案之後，同一支的 `Charles` 才浮出來：「發音非常不清楚」。量測給了句型上的解釋，
但那個解釋是**錯的**：

| clip | Charles 時長 | 後面接什麼 | 使用者判定 |
|---|---|---|---|
| `greet-5` | 0.48s | 逗號 | 接受 |
| `greet-1` | 0.32s | 「的」 | 接受 |
| `intro-1` | 0.24s | 直接接名詞「作品集」 | 不清楚 |

看起來像是「名字後面要有緩衝音節」，於是候選裡準備了 `Charles 的作品集`、
`Charles，作品集`，還有使用者主動提的音譯 `查爾斯`。**但同一句話一字不改重抽，Charles
就拿到 0.40s**，使用者最後選的就是那一支。句型從來不是原因，0.24s 是抽出來的。

音譯沒有被採用還有第二個理由，跟發音無關：`greet-1` 與 `greet-5` 都唸 Charles，只有自介
唸「查爾斯」會讓訪客聽到同一個人有兩種叫法。

### `bye-1`：重抽治不好的那一種

使用者聽到的是「阿掰下次見」。時間戳指出第一個「掰」只有 0.08s，第二個 0.48s——第一個
音節被壓成一個前導碎片。這支跟 `intro-1` 相反，**重抽完全沒用**：同樣文字抽 8 次，7 次
是 0.08s、1 次 0.16s，改寫成「拜拜」也還是 0.08s。這是模型對這個疊字的固定處理。

標點確實能把兩個音節拆開（`掰、掰` 拿到 0.32／0.40），但使用者三支都否決，改要
`Bye Bye`。定案 `Bye bye～，下次見！`，跟 `greet-3` 從「哈囉」換成「Hello」是同一個動作。

### 同一個教訓的第三次，這次代價是使用者要自己把候選要回去

`ack-1`（量測選 A、使用者選 C）與 `intro-1`（量測傾向 normalize 關、使用者選開）之後，
這一輪又來一次，而且形式更糟：`Bye bye` 的候選**早就合成好了**，我拿逐詞時間戳把它
篩掉沒放進試聽頁，使用者是自己問「如果改成 Bye Bye 呢」才把它要回去的。

篩掉它的那個數字本身就是錯的：fish.audio 對拉丁字回報的是**逐詞**時間戳，那批的第二個
bye 被標成 0.04s，短到不可能是真的說話聲。

**規則因此再收一格：量測只能用來排除「可驗證為壞」的東西**（某個音節不是使用者指定的
調值、某支的音高高於他接受過的），**不能用來決定哪些候選值得聽**。候選集合的裁切權在
使用者，不在指標。
